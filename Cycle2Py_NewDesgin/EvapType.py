...
#    input: ISPEC = 1  Exit Superheater
#         : ISPEC = 2  IHX Superheater
#         : ISPEC = 3  Exit Quality
#         : objCP = CoolProp object
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
class EvapType:
    def getObject (self, ISPEC, objCP):
        if (ISPEC == 0):  # Natural Convection
            objEvapType = CondCool_CNat(ISPEC, objCP)

        elif (ISPEC == 1):  # Cross-Flow
            objEvapType = CondCool_CCross(ISPEC, objCP)

        elif (ISPEC == 2):  # Counter-Flow
            objEvapType = CondCool_CCount(ISPEC, objCP)

        else:
            objEvapType = None
            raise ErrorException('ISPEC value error', 'ISPEC1000')
        return objEvapType
...
class Evap_Abstract (ABC):
    ...
    def inter1(self, X, P4, T4, H4, V4, P7, T7, H7, V7, ETHX1):
        ...
        [VGUESS, LCONV] = self.vit(
            T6STAR, P6STAR, A6STAR, B6STAR, VGUESS, True)
        ...
        [VGUESS, LCONV] = self.vit(
            T13STR, P13STR, A13STR, B13STR, VGUESS, False)
        V13STR = VGUESS
    ...
    def inter2(self, X, PA, TAI, HAI, VAI, PB, HBO, TDEW, HDEW, VDEW, ETA):
        ...
        while (ITER <= 100 and HTOL > 0.001):
            ...
            [VGUESS, LCONV] = self.vit(TBI, PA, AA, BB, VGUESS, True)

            VAOSTR = VGUESS

            #[P5, P6, P7, P8] = self.hcvcps (P1, P2, P3, P4)
            # CALL HCVCPS(1,TBI,VAOSTR,X,  HAOSTR,CV,CP,VSND)
            [HAOSTR, CV, CP, VSND] = self.hcvcps(1, TBI, VAOSTR, X)
            ...
            [VGUESS, LCONV] = self.vit(TAI, PB, AA, BB, VGUESS, True)

            VBOSTR = VGUESS

            #[P5, P6, P7, P8] = self.hcvcps (P1, P2, P3, P4)
            # CALL HCVCPS(1,TAI,VBOSTR,X,  HBOSTR,CV,CP,VSND)
            [HBOSTR, CV, CP, VSND] = self.hcvcps(1, TAI, VBOSTR, X)
    ...
    def call_exit_t7(self):
        [self.objData.T[13],
         XQ13,
         self.objData.XL_Temp,
         self.objData.XV_Temp,
         L13,
         self.objData.V[13],
         HL,
         HV] = self.hpin(self.objData.H[13],
                         self.objData.P[13],
                         self.objData.X)
    ...
    def call_exit_p13(self):
        [self.objData.T[13],
         ...
         self.objData.V[13],
         self.objData.HL,
         self.objData.HV] = self.hpin(self.objData.H[13],
                                      self.objData.P[13],
                                      self.objData.X)
...
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# Job 			: Evaporator output for Evap superheat,
#        Evap. output pressure point (7)
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
class EvapSuper (Evap_Abstract):  # ISPEC = 1 Exit Superheater
    ...
    def calc_evap_exit_p7(self):
        ...
        [self.objData.XL_Temp,
         ...
         self.objData.VL[15],
         self.objData.V[15],
         LCRIT] = self.bublt(self.objData.T[15],
                             self.objData.XL_Temp,
                             self.objData.X,
                             False)
    ...
    def iterat_evap_exit_p7(self):
        self.objData.TE[self.objData.JE] = self.objData.T[15] + \
            Data.obj_cdata.DTSUPE
    ...
    def iterat_evap_enthalpy_p7(self):
        ...
        [VGUESS, LCONV] = self.vit(
            self.objData.T[7], self.objData.P[7], A7, B7, VGUESS, False)
        self.objData.V[7] = VGUESS

        [self.objData.H[7], CV, CP, VS] = self.hcvcps(
            1, self.objData.T[7], self.objData.V[7], self.objData.X)
...
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# Job 			: Evaporator output for IHX superheat,
#        Evap. output pressure point (7)
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
class EvapIHX (Evap_Abstract):  # ISPEC = 2 IHX Superheater
    def iterat_call_exit_p13(self):
        self.call_exit_p13()
    ...
    def iterat_evap_exit_p7(self):
        ...
        [VGUESS, LCONV] = self.vit(
            self.objData.T[13], self.objData.P[13], A13, B13, VGUESS, False)
        self.objData.V[13] = VGUESS

        [self.objData.H[13], CV, CP, VS] = self.hcvcps(
            1, self.objData.T[13], self.objData.V[13], self.objData.X)
    ...
    def iterat_evap_enthalpy_p7(self):
        ...
        [self.objData.T[7],
         ...
         self.objData.VV[7],
         HL7,
         HV7] = self.hpin(self.objData.H[7],
                          self.objData.P[7],
                          self.objData.X)
        ...
        self.objData.V[7] = (1.0 - self.objData.XQ[7]) * \
            self.objData.VL[7] + self.objData.XQ[7] * self.objData.VV[7]
        self.objData.TE[self.objData.JE] = self.objData.T[7]
...
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
class EvapQual (Evap_Abstract):  # ISPEC = 3 Exit Quality
    def iterat_call_exit_p13(self):
        self.call_exit_p13()
    ...
    def calc_evap_exit_p7(self):
        ...
        [self.objData.X, self.objData.XV15, TBUB15, VBUB15, VV15, LCRIT] = self.bublp(
            self.objData.P[15], self.objData.X, self.objData.XV15, True)

        self.objData.TE[1] = self.objData.T[15] - \
            (self.objData.T[15] - TBUB15) * (1.0 - Data.obj_cdata.XEXITE)
        self.objData.T[7] = self.objData.TE[1]
        self.objData.P[7] = self.objData.P[15]
