...
class Cycle (Adjlod, HeatExch, CycleUtil, Block2, Data):
    ...
    def __init__(self, obj_parameter, obj_cdata):
        ...
        Cycle.obj_parameter.XV = [[-9999999.9]
                                  * (16 + 1) for i in range(5 + 1)]
        # array(Rows, Cols) = [[0] * Cols for i in range(Rows)]
        Cycle.obj_parameter.XL = [[-9999999.9]
                                  * (16 + 1) for i in range(5 + 1)]

        Cycle.obj_parameter.XL_Temp = [
            0.0] * len(Cycle.obj_parameter.XL)  # in python only
        Cycle.obj_parameter.XV_Temp = [
            0.0] * len(Cycle.obj_parameter.XV)  # in python only
    ...
    def create_basic_obj(self):
        ...
        if (Data.obj_cdata.IMAP == 0):  # Map
            self.objCompType = Comp_Map(
                Cycle.obj_parameter, Data.obj_cdata.FILMAP1)
    ...
    def cycle(self):
        ...
        print(
            "aym Cycle.obj_parameter.TS1,",
            Cycle.obj_parameter.TS1,
            Cycle.obj_parameter.TS1 - 273.11)
        print(
            "aym Cycle.obj_parameter.TS3,",
            Cycle.obj_parameter.TS3,
            Cycle.obj_parameter.TS3 - 273.11)
        print(
            "aym Cycle.obj_parameter.TS5,",
            Cycle.obj_parameter.TS5,
            Cycle.obj_parameter.TS5 - 273.11)
        ...
        if (Cycle.obj_parameter.IRFTYPE != 6):
            IRET = 7

        Cycle.obj_parameter.IRFTYP = Cycle.obj_parameter.IRFTYPE
        if (Cycle.obj_parameter.IRFTYPE == 7):
            Cycle.obj_parameter.IRFTYP = 1
        ...
        if (Data.obj_cdata.ITYPE == 3):
            Data.obj_cdata.ITYPE = 1
        ...
        self.bconst(
            Cycle.obj_parameter.NC,
            Cycle.obj_parameter.IR,
            Cycle.obj_parameter.F)
        ...
        for I in range(1, Cycle.obj_parameter.NC + 1):
            Cycle.obj_parameter.X[I] = Cycle.obj_parameter.XM[I] / \
                Data.WM[I] / WMSUM
            WMAVG = WMAVG + Cycle.obj_parameter.X[I] * Data.WM[I]

        #	INITIAL GUESSES FOR TC AND TE
        #	ASSUME TEMP RISE OF COND IS 0.5 F PER LBM
        Cycle.obj_parameter.TC[1] = Cycle.obj_parameter.TS1 + \
            Data.obj_cdata.MREF / 3.6
        ...
        self.showError(
            "Python, check later, ICNT set in cond method, limit Cycle.obj_parameter.ITMAXC to ",
            Cycle.obj_parameter.ITMAXC)

        while (Data.obj_cdata.IC <=
               Cycle.obj_parameter.ITMAXC and Cycle.obj_parameter.LCCON):

            if (Cycle.obj_parameter.ICAB == 1):
                [Cycle.obj_parameter.TS3,
                 Cycle.obj_parameter.TS5] = self.adjlod(Cycle.obj_parameter.ICYCL,
                                                        Data.obj_cdata.IC,
                                                        Cycle.obj_parameter.TS3,
                                                        Cycle.obj_parameter.TS5,
                                                        Cycle.obj_parameter.FROSTF,
                                                        Cycle.obj_parameter.FROSTF,
                                                        Cycle.obj_parameter.IDFRST)

            Cycle.obj_parameter.T[4] = Cycle.obj_parameter.TC[Cycle.obj_parameter.JC]

            #	find condenser pressure for current guess of TC
            # if bublt routine exceeds the critical point write a warning
            # message and return
            self.showMsg(
                "Iteration number for CONDENSER TEMPERATURE ",
                Data.obj_cdata.IC)

            TBUB4 = Cycle.obj_parameter.TC[Cycle.obj_parameter.JC] + \
                Data.obj_cdata.DTSUBC

            [Cycle.obj_parameter.X,
             Cycle.obj_parameter.XV_Temp,
             Cycle.obj_parameter.P[4],
             VBUB4,
             Cycle.obj_parameter.VV[4],
             LCRIT] = self.bublt(TBUB4,
                                 Cycle.obj_parameter.X,
                                 Cycle.obj_parameter.XV_Temp,
                                 True)
            self.setArr2dCol(Cycle.obj_parameter.XV, 4,
                             Cycle.obj_parameter.XV_Temp)
            ...
            if (Data.obj_cdata.DTSUBC > 0.0):
                VGUESS = VBUB4
                # CALL
                # ESPAR(0,Cycle.obj_parameter.TC[JC],Cycle.obj_parameter.X,
                # A4,B4)
                [A4, B4] = self.espar(
                    0, Cycle.obj_parameter.TC[Cycle.obj_parameter.JC], Cycle.obj_parameter.X)

                [VGUESS,
                 LCONV] = self.vit(Cycle.obj_parameter.TC[Cycle.obj_parameter.JC],
                                   Cycle.obj_parameter.P[4],
                                   A4,
                                   B4,
                                   VGUESS,
            ...
            #	condenser dew point
            Cycle.obj_parameter.P[3] = Cycle.obj_parameter.P[4] + \
                (1.0 - Cycle.obj_parameter.FSUPC) * Cycle.obj_parameter.DPC
            self.showMsg("Conderser Temp TC (C) ",
                         Cycle.obj_parameter.TC[Cycle.obj_parameter.JC] - 273.11)
            ...
            [Cycle.obj_parameter.H[4],
             CV,
             CP,
             VSND] = self.hcvcps(1,
                                 Cycle.obj_parameter.TC[Cycle.obj_parameter.JC],
                                 Cycle.obj_parameter.V[4],
                                 Cycle.obj_parameter.X)
            Cycle.obj_parameter.JE = 1
            Cycle.obj_parameter.LECON = True

            #	ACCOUNT FOR HEAT LOSS FROM LIQUID LINE
            Cycle.obj_parameter.H[16] = Cycle.obj_parameter.H[4] - \
                Data.obj_cdata.CONDHT[Cycle.obj_parameter.NCYC] / Data.obj_cdata.MREF / DUTYR
            Cycle.obj_parameter.P[16] = Cycle.obj_parameter.P[4]

            [Cycle.obj_parameter.T[16],
             Cycle.obj_parameter.XQ[16],
             Cycle.obj_parameter.XL_Temp,
             Cycle.obj_parameter.XV_Temp,
             Cycle.obj_parameter.VL[16],
             Cycle.obj_parameter.VV[16],
             HL16,
             HV16] = self.hpin(Cycle.obj_parameter.H[16],
                               Cycle.obj_parameter.P[16],
                               Cycle.obj_parameter.X)

            self.setArr2dCol(Cycle.obj_parameter.XL, 16,
                             Cycle.obj_parameter.XL_Temp)
            self.setArr2dCol(Cycle.obj_parameter.XV, 16,
                             Cycle.obj_parameter.XV_Temp)

            if (Cycle.obj_parameter.VL[16] == 0.0):
                Cycle.obj_parameter.VL[16] = Cycle.obj_parameter.V[4]
            ...
            while ((Data.obj_cdata.IE <= Cycle.obj_parameter.ITMAXE)
                   and Cycle.obj_parameter.LECON):

                print("\naym Data.obj_cdata.IE=", Data.obj_cdata.IE)
 
                self.showMsg(
                    "Iteration Count for (OR EVAPORATOR OUTLET TEMPERATURE) ",
                    Data.obj_cdata.IE)
                ...
                [Cycle.obj_parameter.X, Cycle.obj_parameter.XV15, TBUB15, VBUB15, VV15, LCRIT] = self.bublp(
                    Cycle.obj_parameter.P[15], Cycle.obj_parameter.X, Cycle.obj_parameter.XV15, True)

                #	determine the bubble and dew point enthalpies
                [Cycle.obj_parameter.H[15], CV, CP, VS] = self.hcvcps(
                    1, Cycle.obj_parameter.T[15], Cycle.obj_parameter.V[15], Cycle.obj_parameter.X)
                [Cycle.obj_parameter.HBUB15, CV, CP, VS] = self.hcvcps(
                    1, TBUB15, VBUB15, Cycle.obj_parameter.X)
                ...
                self.showMsg(
                    "Outlet from fresh food evaporator (C) - point 7 ",
                    Cycle.obj_parameter.T[7] - 273.11)

                #	interchanger for subcooling condenser liquid
                self.objEvapType.iterat_evap_subcool_p7()
                Cycle.obj_parameter.H[6] = Cycle.obj_parameter.H[16] - \
                    Cycle.obj_parameter.QINT
                ...
                [Cycle.obj_parameter.T[6],
                 XQ6,
                 Cycle.obj_parameter.XL_Temp,
                 Cycle.obj_parameter.XV_Temp,
                 Cycle.obj_parameter.V[6],
                 VV6,
                 HL,
                 HV] = self.hpin(Cycle.obj_parameter.H[6],
                                 Cycle.obj_parameter.P[6],
                                 Cycle.obj_parameter.X)
                self.setArr2dCol(Cycle.obj_parameter.XL, 6,
                                 Cycle.obj_parameter.XL_Temp)
                self.setArr2dCol(Cycle.obj_parameter.XV, 6,
                                 Cycle.obj_parameter.XV_Temp)
                ...
                self.showMsg(
                    "Liquid line state after heat loss to cabinet & mullion (C) - point 16 ",
                    Cycle.obj_parameter.T[16] - 273.11)
                self.showMsg(
                    "Superheated gas leaving hight temp interchanger (C) - point 13 ",
                    Cycle.obj_parameter.T[13] - 273.11)

                # FIND CONDITIONS AT EVAPORATOR INLET ASSUMING ISENTHALPIC
                # EXPANSION
                Cycle.obj_parameter.P[5] = Cycle.obj_parameter.P[13] + \
                    Cycle.obj_parameter.DPE
                self.showMsg(
                    "liquid line outlet from high temp interchanger (C) -point 6 ",
                    Cycle.obj_parameter.T[6] - 273.11)

                #	CALL ANALYSIS OF FREEZER SECTION AND THE LOWER
                # INTERCHANGER (LOOKS LIKE A NON-ADIABATIC EXPANSION VALVE TO
                # REST OF SYSTEM
                [Cycle.obj_parameter.H,
                 Cycle.obj_parameter.P,
                 Cycle.obj_parameter.X,
                 Cycle.obj_parameter.T,
                 Cycle.obj_parameter.XQ,
                 Cycle.obj_parameter.XL,
                 Cycle.obj_parameter.XV,
                 Cycle.obj_parameter.VL,
                 Cycle.obj_parameter.VV,
                 Cycle.obj_parameter.HL,
                 Cycle.obj_parameter.HV,
                 Cycle.obj_parameter.TS6,
                 Cycle.obj_parameter.QFREZ] = self.lowevp(Cycle.obj_parameter.ICYCL,
                                                          0,
                                                          Cycle.obj_parameter.H,
                                                          Cycle.obj_parameter.P,
                                                          Cycle.obj_parameter.X,
                                                          ...
                                                          Cycle.obj_parameter.TS3,
                                                          Cycle.obj_parameter.TS5,
                                                          Cycle.obj_parameter.DPF,
                                                          Cycle.obj_parameter.ETHX2)

                #	CALCULATE FRESH FOOD SECTION HEAT EXCHANGE
                Cycle.obj_parameter.PDEWE = Cycle.obj_parameter.P[5] - (
                    1.0 - Cycle.obj_parameter.FSUPE) * Cycle.obj_parameter.DPE
                if (Cycle.obj_parameter.PDEWE > Cycle.obj_parameter.P[5]):
                    Cycle.obj_parameter.PDEWE = Cycle.obj_parameter.P[5]

                [Cycle.obj_parameter.XREF,
                 Cycle.obj_parameter.X,
                 Cycle.obj_parameter.TDEW,
                 Cycle.obj_parameter.VLDEW,
                 Cycle.obj_parameter.VDEW,
                 Cycle.obj_parameter.LCRIT] = self.bublp(Cycle.obj_parameter.PDEWE,
                                                         Cycle.obj_parameter.XREF,
                                                         Cycle.obj_parameter.X,
                                                         False)
                ...
                if (Cycle.obj_parameter.TDEW >= Cycle.obj_parameter.TS3):
                    Cycle.obj_parameter.TDEW = Cycle.obj_parameter.TS3 - 1.0
                if (Cycle.obj_parameter.T[5] >= Cycle.obj_parameter.TS3):
                    Cycle.obj_parameter.T[5] = Cycle.obj_parameter.TS3 - 1.0

                [Cycle.obj_parameter.HDEW,
                 Cycle.obj_parameter.CVRVAP,
                 Cycle.obj_parameter.CPRVAP,
                 Cycle.obj_parameter.VS] = self.hcvcps(3,
                                                       Cycle.obj_parameter.TDEW,
                                                       Cycle.obj_parameter.VDEW,
                                                       Cycle.obj_parameter.X)
                ...
                # FIND DUTY CYCLE, NET CAPACITY AND AVERAGE FREEZER LOAD if
                # ICYCL = 1
                if (Data.obj_cdata.IC != 1):
                    [QFF,
                     QFZ,
                     DUTYR] = self.dutfnd(Cycle.obj_parameter.ICAB,
                                          Cycle.obj_parameter.IRFTYP,
                                          Cycle.obj_parameter.ICYCL,
                                          Cycle.obj_parameter.NCYC,
                                          Cycle.obj_parameter.QFRSH,
                                          Cycle.obj_parameter.QFREZ,
                                          Cycle.obj_parameter.FROSTF,
                                          Cycle.obj_parameter.FROSTF,
                                          Cycle.obj_parameter.TS3,
                                          Cycle.obj_parameter.TS5,
                ...
                if (Cycle.obj_parameter.IRFTYP <= 3):
                    if (Cycle.obj_parameter.ICYCL == 1 and Cycle.obj_parameter.ICAB !=
                            0 and Data.obj_cdata.IFRSH != 0):
                        if (Data.obj_cdata.IC == 1):
                            TIN = 0.15 * TFF + 0.85 * TFZ
                            Cycle.obj_parameter.FF_FRACT = 0.0  # in Python only
                        else:
                            Data.obj_cdata.CAPE = Cycle.obj_parameter.QFRSH / 1.0548 - 3.413 * \
                                Data.obj_cdata.FANE - 3.413 * (Data.obj_cdata.DFSTCYC + Data.obj_cdata.FZCYC)
                            CFMA = Data.obj_cdata.CFME / (1.08 * 1.8961)
                            QFM = QFF + 3.413 * Data.obj_cdata.DUTYC * Data.obj_cdata.FFCYC
                            [TIN, Cycle.obj_parameter.FF_FRACT] = self.mixair(
                                Data.obj_cdata.CAPE, QFM, QFZ, TFF, TFZ, CFMA)

                        Cycle.obj_parameter.TS3 = (TIN + 459.6) / 1.8

                [Cycle.obj_parameter.QFRSH,
                 Cycle.obj_parameter.FSUPE] = self.objEvapCool.calc_heat_temp()

                #	superheating fraction
                FSHOLD = Cycle.obj_parameter.FSUPE
                Cycle.obj_parameter.FSUPE = (
                    FSHOLD + Cycle.obj_parameter.FSUPE) / 2.0

                if (Cycle.obj_parameter.FSUPE > 1.05 * FSHOLD):
                    Cycle.obj_parameter.FSUPE = 1.05 * FSHOLD
                if (Cycle.obj_parameter.FSUPE < 0.95 * FSHOLD):
                    Cycle.obj_parameter.FSUPE = 0.95 * FSHOLD

                #	fresh food section evaporator
                [Cycle.obj_parameter.TS4,
                 Cycle.obj_parameter.TE,
                 Cycle.obj_parameter.JE,
                 ICONE] = self.objEvapCool.frsh(Cycle.obj_parameter.H,
                                                Cycle.obj_parameter.T,
                                                Cycle.obj_parameter.TS3,
                                                Cycle.obj_parameter.TE,
                                                Cycle.obj_parameter.JE,
                ...
                Cycle.obj_parameter.T[15] = Cycle.obj_parameter.T[15] + \
                    Cycle.obj_parameter.TE[2] - Cycle.obj_parameter.T[7]
                # -----------------------------END OF NEW CODE-----------------

                #	calculate the average effectiveness of the ff evaporator
                #	calculate the heat transfer if the refrigerant left at TS3
                VGUESS = Cycle.obj_parameter.VDEW * \
                    Cycle.obj_parameter.TS3 / Cycle.obj_parameter.TDEW
                [AS3, BS3] = self.espar(
                    0, Cycle.obj_parameter.TS3, Cycle.obj_parameter.X)

                [VGUESS, LCONV] = self.vit(
                    Cycle.obj_parameter.TDEW, Cycle.obj_parameter.P[7], AS3, BS3, VGUESS, False)
                VS3 = VGUESS

                [HS3, CV, CP, VS] = self.hcvcps(
                    1, Cycle.obj_parameter.TS3, VS3, Cycle.obj_parameter.X)

                Cycle.obj_parameter.QRMAX = Data.obj_cdata.MREF * \
                    (HS3 - Cycle.obj_parameter.H[5])

                #	calculate the heat transfer if the air left at T[5]
                Cycle.obj_parameter.QAMAX = Data.obj_cdata.CFME * \
                    (Cycle.obj_parameter.TS3 - Cycle.obj_parameter.T[5])
                Cycle.obj_parameter.QMAXE = Cycle.obj_parameter.QAMAX

                if (Cycle.obj_parameter.QRMAX < Cycle.obj_parameter.QAMAX):
                    Cycle.obj_parameter.QMAXE = Cycle.obj_parameter.QRMAX

                Data.obj_cdata.ETAE = Cycle.obj_parameter.QFRSH / Cycle.obj_parameter.QMAXE

                if (ICONE == 1):
                    Cycle.obj_parameter.LECON = False
                if (Cycle.obj_parameter.TE[Cycle.obj_parameter.JE]
                        <= Cycle.obj_parameter.TEMIN):
                    Cycle.obj_parameter.LECON = False
            ...
            self.showMsg(
                "superheated gas leaving the high temp interchanger (C) - point 13",
                Cycle.obj_parameter.T[13] - 273.11)
            ...
            if (Data.obj_cdata.TSPEC > 0.0):
                ...
                VGUESS = Cycle.obj_parameter.V[7] * \
                    Cycle.obj_parameter.T[1] / Cycle.obj_parameter.T[7]

                [A1, B1] = self.espar(
                    0, Cycle.obj_parameter.T[1], Cycle.obj_parameter.X)

                [VGUESS, LCONV] = self.vit(
                    Cycle.obj_parameter.T[1], Cycle.obj_parameter.P[1], A1, B1, VGUESS, False)
                Cycle.obj_parameter.V[1] = VGUESS

                [Cycle.obj_parameter.H[1], CV, CP, VS] = self.hcvcps(
                    1, Cycle.obj_parameter.T[1], Cycle.obj_parameter.V[1], Cycle.obj_parameter.X)
            else:
            ...
            Cycle.obj_parameter.S[1] = self.entrop(
                Cycle.obj_parameter.T[1],
                Cycle.obj_parameter.V[1],
                Cycle.obj_parameter.X)
            Cycle.obj_parameter.P[2] = Cycle.obj_parameter.P[3] + \
                Cycle.obj_parameter.FSUPC * Cycle.obj_parameter.DPC

            self.showMsg(
                "compressor inlet (saturated vapor) (C) - point 1",
                Cycle.obj_parameter.T[1] - 273.11)

            # CALL COMPRESSOR MODEL
            [Cycle.obj_parameter.T,
             HOUT,
             Cycle.obj_parameter.QHILO,
             Cycle.obj_parameter.QCAN,
             VSUC,
             VV2,
             Cycle.obj_parameter.TSUC,
             Cycle.obj_parameter.TDISC,
             GAMA,
             RN,
             Cycle.obj_parameter.ETAS] = self.objCompType.comp_balance()
            ...
            [Cycle.obj_parameter.H[2], CV, CP, VS] = self.hcvcps(
                1, Cycle.obj_parameter.T[2], VV2, Cycle.obj_parameter.X)

            [Cycle.obj_parameter.T[2],
             Cycle.obj_parameter.XQ[2],
             Cycle.obj_parameter.XL_Temp,
             Cycle.obj_parameter.XV_Temp,
             VL2,
             VV2,
             HL2,
             HV2] = self.hpin(Cycle.obj_parameter.H[2],
                              Cycle.obj_parameter.P[2],
                              Cycle.obj_parameter.X)
            self.setArr2dCol(Cycle.obj_parameter.XL, 2,
                             Cycle.obj_parameter.XL_Temp)
            self.setArr2dCol(Cycle.obj_parameter.XV, 2,
                             Cycle.obj_parameter.XV_Temp)
            ...
            if (Cycle.obj_parameter.XQ[2] < 1.0):
                SL2 = self.entrop(
                    Cycle.obj_parameter.T[2], VL2, self.getArr2dCol(
                        Cycle.obj_parameter.XL, 2))
                SV2 = self.entrop(
                    Cycle.obj_parameter.T[2], VV2, self.getArr2dCol(
                        Cycle.obj_parameter.XV, 2))
                Cycle.obj_parameter.S[2] = Cycle.obj_parameter.XQ[2] * \
                    SV2 + (1.0 - Cycle.obj_parameter.XQ[2]) * SL2
            else:
                Cycle.obj_parameter.S[2] = self.entrop(
                    Cycle.obj_parameter.T[2], VV2, Cycle.obj_parameter.X)

            self.showMsg(
                "compressor discharge (C) - point 2",
                Cycle.obj_parameter.T[2] - 273.11)
            ...
            if (ICONC != 1):
                ...
                [Cycle.obj_parameter.XL_Temp,
                 ...
                 VL_notused,
                 VDEW,
                 LCRIT] = self.bublp(Cycle.obj_parameter.P[3],
                                     Cycle.obj_parameter.XL_Temp,
                                     Cycle.obj_parameter.X,
                                     False)
                self.setArr2dCol(Cycle.obj_parameter.XL, 3,
                                 Cycle.obj_parameter.XL_Temp)
                ...
                [HDEW, CVRVAP, CPRVAP, VS] = self.hcvcps(
                    3, TDEW, VDEW, Cycle.obj_parameter.X)
                ...
                #
                PBUB = Cycle.obj_parameter.P[4] + \
                    Cycle.obj_parameter.DPC * Cycle.obj_parameter.FSUBC

                [Cycle.obj_parameter.X, Cycle.obj_parameter.XV_Temp, TBUB, VBUB, V4, LCRIT] = self.bublp(
                    PBUB, Cycle.obj_parameter.X, Cycle.obj_parameter.XV_Temp, True)
                self.setArr2dCol(Cycle.obj_parameter.XV, 4,
                                 Cycle.obj_parameter.XV_Temp)

                [HBUB, CVRLIQ, CPRLIQ, VS] = self.hcvcps(
                    3, TBUB, VBUB, Cycle.obj_parameter.X)
                ...
                #
                HDROP = Data.obj_cdata.CONDVP[Cycle.obj_parameter.NCYC] / \
                    Data.obj_cdata.MREF / Data.obj_cdata.DUTYC
                ...
                [Cycle.obj_parameter.T[14],
                 Cycle.obj_parameter.XQ[14],
                 Cycle.obj_parameter.XL_Temp,
                 Cycle.obj_parameter.XV_Temp,
                 Cycle.obj_parameter.VL[14],
                 Cycle.obj_parameter.VV[14],
                 HL14,
                 HV14] = self.hpin(Cycle.obj_parameter.H[14],
                                   Cycle.obj_parameter.P[2],
                                   Cycle.obj_parameter.X)

                self.setArr2dCol(Cycle.obj_parameter.XL, 14,
                                 Cycle.obj_parameter.XL_Temp)
                self.setArr2dCol(Cycle.obj_parameter.XV, 14,
                                 Cycle.obj_parameter.XV_Temp)

                Cycle.obj_parameter.V[14] = (
                    1.0 - Cycle.obj_parameter.XQ[14]) * Cycle.obj_parameter.VL[14] + Cycle.obj_parameter.XQ[14] * Cycle.obj_parameter.VV[14]

                [CPRVAP, QDSC, QTPC, QSCC, QTOTC, Cycle.obj_parameter.FSUPC,
                    Cycle.obj_parameter.FSUBC] = self.objCondType.cond_balance(CPRLIQ)
                ...
                [Cycle.obj_parameter.TS2,
                 Cycle.obj_parameter.TC,
                 Cycle.obj_parameter.JC,
                 ICONC] = self.objCondType.cond(Cycle.obj_parameter.T,
                                                Cycle.obj_parameter.H,
                                                Cycle.obj_parameter.TS1,
                                                Cycle.obj_parameter.TC,
                                                QDSC,
                ...
                if (Cycle.obj_parameter.TS1 < Cycle.obj_parameter.T[4]):
                    ...
                    # CALL
                    # ESPAR(0,Cycle.obj_parameter.TS1,Cycle.obj_parameter.X,AS1,BS1)
                    [AS1, BS1] = self.espar(
                        0, Cycle.obj_parameter.TS1, Cycle.obj_parameter.X)

                    #[P5, P7] = self.vit (P1, P2, P3, P4, P5, P6)
                    # CALL
                    # VIT(Cycle.obj_parameter.TS1,Cycle.obj_parameter.P[4],AS1,BS1,VGUESS,True,LCONV)
                    [VGUESS, LCONV] = self.vit(
                        Cycle.obj_parameter.TS1, Cycle.obj_parameter.P[4], AS1, BS1, VGUESS, True)
                    VS1 = VGUESS
                else:
                    VS1 = Cycle.obj_parameter.V[4]

                # CALL
                # HCVCPS(1,Cycle.obj_parameter.TS1,VS1,Cycle.obj_parameter.X,
                # HS1,CV,CP,VS)
                [HS1, CV, CP, VS] = self.hcvcps(
                    1, Cycle.obj_parameter.TS1, VS1, Cycle.obj_parameter.X)

                QRMAX = Data.obj_cdata.MREF * (Cycle.obj_parameter.H[14] - HS1)
                #
                #	CALCULATE THE HEAT TRANSFER if THE AIR LEFT AT T[14]
                #
                QAMAX = Data.obj_cdata.CFMC * \
                    (Cycle.obj_parameter.T[14] - Cycle.obj_parameter.TS1)
                QMAXC = QAMAX
                if (QRMAX < QAMAX):
                    QMAXC = QRMAX
            ...
            # if (LQUIT): Cycle.obj_parameter.LCCON = False #if key pressed in
            # lowevp ,ethod then exit, cancel in Python
        ...
        #	SPECIFIC VOLUMES
        #
        Cycle.obj_parameter.V[5] = min(
            1.0, 1.0 - Cycle.obj_parameter.XQ[5]) * Cycle.obj_parameter.VL[5] + max(
            0.0, Cycle.obj_parameter.XQ[5]) * Cycle.obj_parameter.VV[5]
        Cycle.obj_parameter.V[8] = min(
            1.0, 1.0 - Cycle.obj_parameter.XQ[8]) * Cycle.obj_parameter.VL[8] + max(
            0.0, Cycle.obj_parameter.XQ[8]) * Cycle.obj_parameter.VV[8]
        Cycle.obj_parameter.V[9] = min(
            1.0, 1.0 - Cycle.obj_parameter.XQ[9]) * Cycle.obj_parameter.VL[9] + max(
            0.0, Cycle.obj_parameter.XQ[9]) * Cycle.obj_parameter.VV[9]
        Cycle.obj_parameter.V[10] = min(
            1.0, 1.0 - Cycle.obj_parameter.XQ[10]) * Cycle.obj_parameter.VL[10] + max(
            0.0, Cycle.obj_parameter.XQ[10]) * Cycle.obj_parameter.VV[10]
        #
        #	QUALITIES
        #
        ...
        for J in range(3, 16 + 1):  # DO WHILE (J  <=  16)
            if (J != 5):
                Cycle.obj_parameter.S[J] = self.entrop(
                    Cycle.obj_parameter.T[J], Cycle.obj_parameter.V[J], Cycle.obj_parameter.X)
            #J = J + 1
        # END DO

        SL5 = self.entrop(
            Cycle.obj_parameter.T[5],
            Cycle.obj_parameter.VL[5],
            self.getArr2dCol(
                Cycle.obj_parameter.XL,
                5))  # XL[1][5]
        SV5 = self.entrop(
            Cycle.obj_parameter.T[5],
            Cycle.obj_parameter.VV[5],
            self.getArr2dCol(
                Cycle.obj_parameter.XV,
                5))  # XV[1][5]
        Cycle.obj_parameter.S[5] = min(1.0,
                                       1.0 - Cycle.obj_parameter.XQ[5]) * SL5 + max(0.0,
                                                                                    Cycle.obj_parameter.XQ[5]) * SV5

        SL8 = self.entrop(
            Cycle.obj_parameter.T[8],
            Cycle.obj_parameter.VL[8],
            self.getArr2dCol(
                Cycle.obj_parameter.XL,
                8))  # XL[1][8]
        SV8 = self.entrop(
            Cycle.obj_parameter.T[8],
            Cycle.obj_parameter.VV[8],
            self.getArr2dCol(
                Cycle.obj_parameter.XV,
                8))  # XV[1][8]
        Cycle.obj_parameter.S[8] = min(1.0,
                                       1.0 - Cycle.obj_parameter.XQ[8]) * SL8 + max(0.0,
                                                                                    Cycle.obj_parameter.XQ[8]) * SV8

        SL9 = self.entrop(
            Cycle.obj_parameter.T[9],
            Cycle.obj_parameter.VL[9],
            self.getArr2dCol(
                Cycle.obj_parameter.XL,
                9))  # XL[1][9]
        SV9 = self.entrop(
            Cycle.obj_parameter.T[9],
            Cycle.obj_parameter.VV[9],
            self.getArr2dCol(
                Cycle.obj_parameter.XV,
                9))  # XV[1][9]
        Cycle.obj_parameter.S[9] = min(1.0,
                                       1.0 - Cycle.obj_parameter.XQ[9]) * SL9 + max(0.0,
                                                                                    Cycle.obj_parameter.XQ[9]) * SV9
        #
        ...
        for J in range(1, 16 + 1):  # DO WHILE (J  <=  16)
            ...
            for I in range(
                    1, Cycle.obj_parameter.NC + 1):  # DO WHILE (I  <=  NC)
                Cycle.obj_parameter.WMAVGL[J] = Cycle.obj_parameter.WMAVGL[J] + (
                    Cycle.obj_parameter.XL[I][J]) * (Data.WM[I])
                Cycle.obj_parameter.WMAVGV[J] = Cycle.obj_parameter.WMAVGV[J] + (
                    Cycle.obj_parameter.XV[I][J]) * (Data.WM[I])

            for I in range(
                    1, Cycle.obj_parameter.NC + 1):  # DO WHILE (I  <=  NC)
                if (Cycle.obj_parameter.XL[I][J] > 0.0):
                    Cycle.obj_parameter.XL[I][J] = Cycle.obj_parameter.XL[I][J] * \
                        Data.WM[I] / Cycle.obj_parameter.WMAVGL[J]
                if (Cycle.obj_parameter.XV[I][J] > 0.0):
                    Cycle.obj_parameter.XV[I][J] = Cycle.obj_parameter.XV[I][J] * \
                        Data.WM[I] / Cycle.obj_parameter.WMAVGV[J]
        ...
        VSUC = VSUC / WMAVG
        Data.obj_cdata.W = (
            HOUT - Cycle.obj_parameter.H[1]) / (1.0 - Cycle.obj_parameter.QCAN)
        Data.obj_cdata.QE = Cycle.obj_parameter.H[7] - Cycle.obj_parameter.H[5]
        Cycle.obj_parameter.QC = Cycle.obj_parameter.H[14] - \
            Cycle.obj_parameter.H[4]

        Data.obj_cdata.QZ = Cycle.obj_parameter.H[9] - Cycle.obj_parameter.H[8]
        Data.obj_cdata.COPR = (
            Data.obj_cdata.QE + Data.obj_cdata.QZ) / Data.obj_cdata.W

        TH = Cycle.obj_parameter.TS1
        TL1 = Cycle.obj_parameter.TS3
        TL2 = Cycle.obj_parameter.TS5
        DENOM = TH * (Data.obj_cdata.QE * (1. / TL1 - 1. / TH) +
                      Data.obj_cdata.QZ * (1. / TL2 - 1. / TH))
        COPI = (Data.obj_cdata.QE + Data.obj_cdata.QZ) / DENOM
        Cycle.obj_parameter.PR = Cycle.obj_parameter.P[2] / \
            Cycle.obj_parameter.P[1]
        Cycle.obj_parameter.TSUPC = Cycle.obj_parameter.T[2] - \
            Cycle.obj_parameter.T[3]
        ...
        if (Data.obj_cdata.I_CYCLE == 0):
            ...
            TMID_COND = (
                Cycle.obj_parameter.T[3] + Cycle.obj_parameter.T[11]) / 2.0
            TMID_EVAP = (
                Cycle.obj_parameter.T[8] + Cycle.obj_parameter.T[9]) / 2.0
            Data.obj_cdata.CORR_COP = self.cyclos(
                Data.obj_cdata.I_VALVE, Data.obj_cdata.T_CYCLE)
    ...
    def dutfnd(
        ...
        if (TS5 > -300.0):  # Freezer evaporator
            TRFZ = (T[8] + T[9]) / 2.0
            Data.obj_cdata.Q_FZ_IN_WALL = 1.8 * \
                Data.obj_cdata.UA_FZ * (TENV - TS5)
            Data.obj_cdata.Q_ML_IN_WALL = 1.8 * \
                Data.obj_cdata.UA_ML * (Cycle.obj_parameter.TS3 - TS5)

            Data.obj_cdata.CAPZ_IN_WALL = 1.8 * \
                Data.obj_cdata.UA_FZ * (TENV - TRFZ)
            Data.obj_cdata.CAPM_IN_WALL = 1.8 * \
                Data.obj_cdata.UA_ML * (Cycle.obj_parameter.TS3 - TRFZ)
        ...
        TRFF = (T[5] + T[7]) / 2.0
        Data.obj_cdata.Q_FF_IN_WALL = 1.8 * \
            Data.obj_cdata.UA_FF * (TENV - Cycle.obj_parameter.TS3)
        Data.obj_cdata.CAPE_IN_WALL = 1.8 * \
            Data.obj_cdata.UA_FF * (TENV - TRFF)
        Data.obj_cdata.CONDF_IN_WALL = 1.8 * \
            Data.obj_cdata.UA_FF_CND * (TCND - TENV)
        Data.obj_cdata.CONDZ_IN_WALL = 1.8 * \
            Data.obj_cdata.UA_FZ_CND * (TCND - TENV)
        #
        ...
        if IRFTYP in [1, 3]:
            if (ICYCL == 1):
                ...
                if (Data.obj_cdata.DUTYC > 1.0):
                    Data.obj_cdata.DUTYC = 1.0
            ...
            if (ICYCL == 2):
                ...
                Data.obj_cdata.DUTYC = min(
                    Data.obj_cdata.DUTYE, Data.obj_cdata.DUTYZ)
            ...
            if (ICYCL == 3):
                if (N == 1):
                    ...
                    Data.obj_cdata.CAPZ = QFRSH / 1.0548 - 3.413 * Data.obj_cdata.FANE - \
                        3.413 * (Data.obj_cdata.DFSTCYC + Data.obj_cdata.FZCYC)
                    Data.obj_cdata.DUTYZ = QFZ / Data.obj_cdata.CAPZ
                    ...
                    Data.obj_cdata.CAPE = QFRSH / 1.0548 - 3.413 * \
                        (Data.obj_cdata.FANE + Data.obj_cdata.FFCYC) + Data.obj_cdata.Q_FF_IN_WALL - Data.obj_cdata.CAPE_IN_WALL
                    QFF = QFF - Cycle.obj_parameter.FROSTF
                    Data.obj_cdata.DUTYE = QFF / Data.obj_cdata.CAPE
    ...
    def progrs(self, ICYCL, H, HOUT, WM, FLOW, QCAN):
        ...
        if (ICYCL == 2):
            ...
            self.showMsg('Fresh Food QE:', int(QE / 3.413))
            self.showMsg('Compressor W:', int(W / 3.413))
            self.showMsg('COP:', COPR)

        else:
            self.showMsg('Mass Flow:', FLOW / 2.2046)  # ,5,1
            self.showMsg('Evaporator QE:', int(QE / 3.413))
            self.showMsg('Compressor W:', int(W / 3.413))
            self.showMsg('COP:', COPR)
        return
        self.showMsg("====================", 0, True)
