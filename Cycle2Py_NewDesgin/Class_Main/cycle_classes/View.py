...
class View:
    def __init__(self, dt, str_file_cycle, str_path_cycle):
    ...
    def show_rep(self):
        print("\n\n\tShow all results in View Class ... later ...\n\n")
...
class ViewCycle:
    ...
    def show_rep(self):
		...
		objCycOut.write_or_terminate('THE REFRIGERANT MIXTURE CONSISTS OF  %4.0f OF %s'    %(X2, self.dt.HREF[ self.dt.IR[1] ])  )

		if (self.dt.NC >  1) :
			for I in range (2, self.dt.NC+1) :
				X2 = 100.0 * self.ds.XM[I]
				objCycOut.write_or_terminate('THE REFRIGERANT MIXTURE CONSISTS OF  %4.0f OF %s'    %(X2, self.dt.HREF[ self.dt.IR[1] ])  ) # fixed in python
		objCycOut.write_or_terminate(" ")
		'''
        ...
        # if self.ds.str_err != "":
            # objCycOut.write_or_terminate(self.ds.str_err)
        ...
        objCycOut.write_or_terminate(""", STATE
                                      , T(C), T(C)
                                      , P, H, V, S
                                      , XL, XV, XQ""")
                                      
        objCycOut.write_or_terminate(""", , AIR,   REF
        ...
        if (self.dt.IMAP == 1):
            objCycOut.write_or_terminate(
                'ESTIMATED COMPRESSION EFFICIENCY, %9.2f, (COMPRESSOR EER MODEL)' %
                (self.ds.ETAS))
                
            objCycOut.write_or_terminate(
                'ESTIMATED MOTOR-PUMP EFFICIENCY,  %9.2f, (COMPRESSOR EER MODEL)' %
                (self.ds.EFFC / self.ds.ETAS))
                
            objCycOut.write_or_terminate(
                'ESTIMATED CLEARANCE VOLUME,       %9.2f, (COMPRESSOR EER MODEL)' %
                (self.dt.CE))
                
            objCycOut.write_or_terminate(
                'ESTIMATED SHELL LOSS,       %9.2f, (COMPRESSOR EER MODEL)' %
                (self.ds.QCAN))
                
            objCycOut.write_or_terminate(
                'ESTIMATED DISC TUBE HEAT LOSS,       %9.2f, (COMPRESSOR EER MODEL)' %
                (self.ds.QHILO))
