...
class CycleApp( Frame ):
	...
	def __init__( self ):
		...
		self.button1 = Button( self, text = "Type 1-" + CycleApp.lst_Cycle_Type[0], width = 25,
							command=partial(self.set_config, 1) )

		self.button2 = Button( self, text = "Type 2-" + CycleApp.lst_Cycle_Type[1], width = 25,
							command=partial(self.set_config, 2) )

		self.button3 = Button( self, text = "Type 3-" + CycleApp.lst_Cycle_Type[2], width = 25,
							command=partial(self.set_config, 3) )
							
		self.button4 = Button( self, text = "Type 4-" + CycleApp.lst_Cycle_Type[3], width = 25,
							command=partial(self.set_config, 0) )
	...
	def set_config(self, int_config =0):
		...
		if int_config != 0:
			self.str_file_cyc_in    = str(int_config) + "-" + CycleApp.FILE_INP_CYC
			self.str_file_cyc_out   = str(int_config) + "-" + CycleApp.FILE_OUT_CYC
		...
		if not self.isFileExisits  ( self.str_file_cyc_in, strPath):
			mb.showerror("Error", "Cannot find file:" +  strPath + "\\" + self.str_file_cyc_in)
			return
		
		self.obj_start.set_filenames( self.str_file_cyc_in,	self.str_file_cyc_out, \
									  strPath, strPath)
		...
		mb.showinfo('Cycle. App Done Succufully', ""\
			...
			+ "Input File: "     + strPath + "\\" + self.obj_start.FILE_CYC_INPUT + "\n" 	\
			+ "File create on  " + strPath + "\\" + self.obj_start.FILE_CYC_OUTPUT + "\n\n"	\
			+ "Configration: " + str(self.obj_start.obj_data.IRFTYP) + "\n"				\
			+ "Cycle        : " + str(self.obj_start.obj_data.ICYCL)  \
		...
		print ("|                                                |")
		print ("            Configration: " + str ( self.obj_start.obj_data.IRFTYP))
