...
class CondCool_Abstract (ABC, HeatExch, Data):
    ...
    # =.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.==.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=
    def cond(self, T, H, TS1, TC, QCONDS, QCONDC, QSCC, JC):
        ...
        # Python Commnet : MREF-Initial Guess For Refrigerant Mas Flow Rate
        # (kg/hr)
        QREF = Data.obj_cdata.MREF * (H[14] - H[4])
        ...
        if(JC <= 2):
            ...
            if ((TCOUT > TC[1] and TC[1] > TC[2]) or (
                    TCOUT < TC[1] and TC[1] < TC[2])):
                TCNEW = 0.5 * (TC[1] + TC[2])
            else:
                TCNEW = TCOUT
...
class CondCool_CNat (CondCool_Abstract):  # Data.obj_cdata.ICOND= 0
    ...
    def cnat(self, TS1, TS3, TS5, T14, H14, T3, H3, T, TBUB, HBUB, CPRLIQ):
        ...
        #
        # Set up the hdew and tdew parameters.  account for a wet gas entering
        # the condenser
        ...
        Data.obj_cdata.Q_CND_FF = 1.8 * \
            Data.obj_cdata.UA_FF_CND * (TCND - TS3) * 1.0548
        Data.obj_cdata.Q_CND_FZ = 1.8 * \
            Data.obj_cdata.UA_FZ_CND * (TCND - TS5) * 1.0548

        TFZ = 0.5 * (T[8] + T[9])
        TFF = 0.5 * (T[5] + T[12])
        Data.obj_cdata.Q_HXS_FF = 1.8 * \
            Data.obj_cdata.UA_FF_HXS * (TCND - TFF) * 1.0548
        Data.obj_cdata.Q_HXS_FZ = 1.8 * \
            Data.obj_cdata.UA_FZ_HXS * (TCND - TFZ) * 1.0548

        if (TS5 < -290.0):
            Data.obj_cdata.Q_CND_FZ = 0
        if (TS5 < -290.0):
            Data.obj_cdata.Q_HXS_FZ = 0
        ...
        if (ADSNEC > Data.obj_cdata.ATOTC):
            ADSNEC = Data.obj_cdata.ATOTC
        ...
        Data.obj_cdata.UACOND = Data.obj_cdata.ATOTC * \
            (FSUP * Data.obj_cdata.UDSC + (1.0 - FSUP) * Data.obj_cdata.UTPC)
...
class CondCool_CCross (CondCool_Abstract):  # Data.obj_cdata.ICOND== 1
    ...
    def ccross(
        ...
        if(Data.obj_cdata.DTSUBC > 0.0):
            ...
            [EFFSCC, DEXDAR] = self.exf(
                2, Data.obj_cdata.ATOTC, Data.obj_cdata.USCC, CMINSC, CMAXSC)
            # CALL
            # EXF(2,Data.obj_cdata.ATOTC,Data.obj_cdata.USCC,CMINSC,CMAXSC,
            # EFFSCC,DEXDAR)
            ...
            if(EFFSCC <= EFF_SUB):  # Need more area
                ...
                while (LOOKING_FOR_AREA):
                    ...
                    [EFFSCC, DEXDAR] = self.exf(
                        2, ASCC, Data.obj_cdata.USCC, CMINSC, CMAXSC)
                    # CALL EXF(2,ASCC,Data.obj_cdata.USCC,CMINSC,CMAXSC,
                    # EFFSCC,DEXDAR)
        ...
        for N in range(1, NUM_ZONE + 1):  # DO WHILE (N  <=  NUM_ZONE)
            ...
            if(HAVE_NOT_USED_FULL_AREA):
                ...
                [EFFTPC, DEXDAR] = self.exf(
                    2, ALEFT, Data.obj_cdata.UTPC, CMINTP, CMAXTP)
                # CALL EXF(2,ALEFT,Data.obj_cdata.UTPC,CMINTP,CMAXTP
                # ,EFFTPC,DEXDAR)
                ...
                if(EFFTPC <= EFF_TPC or ENTERS_WET):  # Need more area
                    ...
                    while (LOOKING_FOR_AREA):
                        ...
                        [EFFTPC, DEXDAR] = self.exf(
                            2, ADUM, Data.obj_cdata.UTPC, CMINTP, CMAXTP)
                        # CALL EXF(2,ADUM,Data.obj_cdata.UTPC,CMINTP,CMAXTP,
                        # EFFTPC,DEXDAR)
...
class CondCool_CCount(CondCool_Abstract):  # Data.obj_cdata.ICOND== 2
    ...
    def ccount(
        ...
        if(Data.obj_cdata.DTSUBC > 0.0):
            ...
            EFF_SUB = QSUB / QMAX
            [EFFSCC, DEXDAR] = self.exf(
                1, Data.obj_cdata.ATOTC, Data.obj_cdata.USCC, CMINSC, CMAXSC)
            # CALL
            # EXF(1,Data.obj_cdata.ATOTC,Data.obj_cdata.USCC,CMINSC,CMAXSC,
            # EFFSCC,DEXDAR)
            ...
            if(EFFSCC < EFF_SUB):  # Need more area
                ...
                while (LOOKING_FOR_AREA):
                    [EFFSCC, DEXDAR] = self.exf(
                        1, ASCC, Data.obj_cdata.USCC, CMINSC, CMAXSC)
                    # CALL EXF(1,ASCC,Data.obj_cdata.USCC,CMINSC,CMAXSC,
                    # EFFSCC,DEXDAR)
        ...
        for N in range(1, NUM_ZONE):  # DO WHILE (N  <  NUM_ZONE)
            ...
            if(HAVE_NOT_USED_FULL_AREA):
                ...
                EFF_TPC = QDUM / QMAX
                [EFFTPC, DEXDAR] = self.exf(
                    1, ALEFT, Data.obj_cdata.UTPC, CMINTP, CMAXTP)
                # CALL EXF(1,ALEFT,Data.obj_cdata.UTPC,CMINTP,CMAXTP,
                # EFFTPC,DEXDAR)
                ...
                if(EFFTPC < EFF_TPC or (ENTERS_WET .AND. N == NUM_ZONE)):
                    ...
                    while (LOOKING_FOR_AREA):
                        ILOOK = ILOOK + 1
                        [EFFTPC, DEXDAR] = self.exf(
                            1, ADUM, Data.obj_cdata.UTPC, CMINTP, CMAXTP)
                        # CALL EXF(1,ADUM,Data.obj_cdata.UTPC,CMINTP,CMAXTP,
                        # EFFTPC,DEXDAR)
        ...
        if(HAVE_NOT_USED_FULL_AREA):
            ...
            #
            [EFFDSC, DEXDAR] = self.exf(
                1, ALEFT, Data.obj_cdata.UDSC, CMINDS, CMAXDS)
            # CALL EXF(1,ALEFT,Data.obj_cdata.UDSC,CMINDS,CMAXDS
            # ,EFFDSC,DEXDAR)
            QDSC = CMINDS * EFFDSC * (T2 - TAIR)
