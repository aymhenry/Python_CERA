...
class EvapCool_Abstract (ABC, HeatExch, Data):
    ...
    def frsh(self, H, T, TS3, TE, JE, QFRSH):
        ...
        ALPHA = QFRSH / (Data.obj_cdata.MREF * (H[7] - H[5]))
...
class EvapCool_FFNat (EvapCool_Abstract):  # Data.obj_cdata.IFRSH== 0
    ...
    def ffnat(self, T5, H5, T7, TDEW, HDEW, TS3, CPRVAP, IRFTYP):
        ...
        Q_IN_WALL = 1.0548 * 1.8 * Data.obj_cdata.UA_FF * \
            (TENV - TAVE) / AEVAP + Data.obj_cdata.Q_HXS_FF / AEVAP
    ...
    def ffcross(
        ...
        for N in range(1, NUM_ZONE):  # DO WHILE (N  <=  NUM_ZONE)
            ...
            if(HAVE_NOT_USED_FULL_AREA):
                ...
                [EFFTPC, DEXDAR] = self.exf(
                    2, ALEFT, Data.obj_cdata.UTPE, CMINTP, CMAXTP)
                ...
                if(EFFTPC <= EFF_TPC):  # Need more area
                    ...
                    while (LOOKING_FOR_AREA):
                        ...
                        CAIR = (ADUM / Data.obj_cdata.ATOTE) * \
                            Data.obj_cdata.CFME
                        if(CAIR <= CRTP):
                            CMINTP = CAIR
                            CMAXTP = CRTP
                        ...
                        [EFFTPC, DEXDAR] = self.exf(
                            2, ADUM, Data.obj_cdata.UTPE, CMINTP, CMAXTP)
        ...
        if(HAVE_NOT_USED_FULL_AREA):
            ...
            [EFFDSC, DEXDAR] = self.exf(
                2, ALEFT, Data.obj_cdata.USUPE, CMINDS, CMAXDS)
        ...
        QTOTE = QSUP + QTPC
        Data.obj_cdata.UAFF = Data.obj_cdata.ATOTE * FSUPE * Data.obj_cdata.USUPE + \
            Data.obj_cdata.ATOTE * (1.0 - FSUPE) * Data.obj_cdata.UTPE
        ...
        while (not CONVERGED):
            ...
            for N in range(1, NUM_ZONE + 1):  # DO WHILE (N  <=  NUM_ZONE)
                ...
                if(HAVE_NOT_USED_FULL_AREA):
                    ...
                    [EFFTPC, DEXDAR] = self.exf(
                        1, ALEFT, Data.obj_cdata.UTPE, CMINTP, CMAXTP)
                    # CALL EXF(1,ALEFT,Data.obj_cdata.UTPE,CMINTP,CMAXTP,
                    # EFFTPC,DEXDAR)
                    if(EFFTPC <= EFF_TPC):
                        ATPC = ATPC + ALEFT
                        HAVE_NOT_USED_FULL_AREA = False
                        ...
                        while (LOOKING_FOR_AREA):
                            ILOOK = ILOOK + 1
                            [EFFTPC, DEXDAR] = self.exf(
                                1, ADUM, Data.obj_cdata.UTPE, CMINTP, CMAXTP)
                            # CALL
                            # EXF(1,ADUM,Data.obj_cdata.UTPE,CMINTP,CMAXTP,
                            # EFFTPC,DEXDAR)
            ...
            if(HAVE_NOT_USED_FULL_AREA):
                ...
                #
                [EFFDSC, DEXDAR] = self.exf(
                    1, ALEFT, Data.obj_cdata.USUPE, CMINDS, CMAXDS)
                # CALL EXF(1,ALEFT,Data.obj_cdata.USUPE,CMINDS,CMAXDS,
                # EFFDSC,DEXDAR)
                QSUP = CMINDS * EFFDSC * (TS3 - TDEW)
                TAIR = TAIR + QSUP / CAIR
