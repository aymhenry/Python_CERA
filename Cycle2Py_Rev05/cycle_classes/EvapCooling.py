...
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
class EvapCool_Abstract (ABC, HeatExch, Data):
	...
	def frsh(self, H,T,TS3, TE,JE,QFRSH):
		...
		ALPHA = QFRSH/(Data.obj_cdata.MREF*( H[7] - H[5]) ) # Python: MREF:Initial Guess For Refrigerant Mas Flow Rate (kg/hr)
		...
		TS4 = TS3 - QFRSH/Data.obj_cdata.CFME
		...
		if (ERROR  <  Data.obj_cdata.TOL_FRSH): ICONE = 1
...
class EvapCool_FFNat (EvapCool_Abstract): #Data.obj_cdata.IFRSH== 0 
	...
	def ffnat (self, T5,H5,T7,  TDEW,HDEW,TS3,  CPRVAP, IRFTYP):
		...
		QTPNEC = Data.obj_cdata.MREF * (HDEW-H5)
		...
		if (ATPNEC  <  AEVAP):
			QTPE = QTPNEC
			ASUPE = AEVAP - ATPNEC
			QSUPMX = Data.obj_cdata.MREF*CPRVAP*(TAIR-TDEW)
		...
		Data.obj_cdata.UAFF = UAIR * AEVAP     # extra output to common
...
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
class EvapCool_FFCross (EvapCool_Abstract): #Data.obj_cdata.IFRSH== 1
	#=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.==.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=
	def calc_heat_temp (self):
		return self.ffcross (self.objData.H[5], self.objData.T[5], 
			self.objData.HDEW, self.objData.TDEW, self.objData.TS3, \
			self.objData.CPRVAP, self.objData.P[5], self.objData.P[7], self.objData.X, Data.obj_cdata.N_EVAP)
	...
	def ffcross(self, H5_S,T5_S,  HDEW_S,TDEW_S,TS3,  CPR,PIN,POUT, X,NUM_ZONE):
		...
		for N in range (1, NUM_ZONE): #	DO WHILE (N  <=  NUM_ZONE)
			...
			if(HAVE_NOT_USED_FULL_AREA) :
				...
				#
				CAIR = (ALEFT/ Data.obj_cdata.ATOTE) * Data.obj_cdata.CFME
				if (CAIR  <=  CRTP) :
					CMINTP = CAIR
					CMAXTP = CRTP
				else:
				...
				#
				QMAX = CMINTP*(TAIR - T5)
				QDUM = Data.obj_cdata.MREF * (HDEW - H5)

				EFF_TPC = QDUM/QMAX
				[EFFTPC,DEXDAR] = self.exf (2,ALEFT, Data.obj_cdata.UTPE,CMINTP,CMAXTP)
				...
				if(EFFTPC  <=  EFF_TPC):                   #Need more area
					...
					while (LOOKING_FOR_AREA):
						...
						CAIR = (ADUM/ Data.obj_cdata.ATOTE) * Data.obj_cdata.CFME
						if(CAIR  <=  CRTP):
							CMINTP = CAIR
							CMAXTP = CRTP
						else:
						...
						[ EFFTPC,DEXDAR] = self.exf (2,ADUM, Data.obj_cdata.UTPE,CMINTP,CMAXTP)
			...
			#END if
				...
				ALEFT = Data.obj_cdata.ATOTE - ATPC
				H5 = H5 + DELH
				T5 = TDEW
				P5 = P5 + DELP
				N = N + 1
		...
		if(HAVE_NOT_USED_FULL_AREA):
			...
			#
			#[P6, P7] = self.exf (P1 ... P5)
			[EFFDSC, DEXDAR] = self.exf (2,ALEFT,Data.obj_cdata.USUPE,CMINDS,CMAXDS)
		...
		FSUPE = ASUP/ Data.obj_cdata.ATOTE
		QTOTE = QSUP + QTPC
		Data.obj_cdata.UAFF = Data.obj_cdata.ATOTE * FSUPE * Data.obj_cdata.USUPE + Data.obj_cdata.ATOTE * (1.0 - FSUPE) * Data.obj_cdata.UTPE
...
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
class EvapCool_FFCount (EvapCool_Abstract): #Data.obj_cdata.IFRSH== 2
	#=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.==.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=
	def calc_heat_temp (self):
		...
		while ( not  CONVERGED):
			...
			for N in range (1, NUM_ZONE + 1) : #DO WHILE (N  <=  NUM_ZONE)
				...
				if(HAVE_NOT_USED_FULL_AREA) :
					...
					[EFFTPC,DEXDAR] = self.exf (1,ALEFT,Data.obj_cdata.UTPE,CMINTP,CMAXTP)
					#CALL EXF(1,ALEFT,Data.obj_cdata.UTPE,CMINTP,CMAXTP, EFFTPC,DEXDAR)
					if(EFFTPC  <=  EFF_TPC) :
						ATPC = ATPC + ALEFT
						HAVE_NOT_USED_FULL_AREA = False
						#
						...
						while (LOOKING_FOR_AREA):
							...
							if(abs(DAREA)  <=  0.001*Data.obj_cdata.ATOTE) :
								LOOKING_FOR_AREA = False
								continue
							#End if
			...
			if(HAVE_NOT_USED_FULL_AREA) :
				...
				#        DETERMINE THE NET HEAT TRANSFER
				#
				[EFFDSC,DEXDAR] = self.exf (1,ALEFT,Data.obj_cdata.USUPE,CMINDS,CMAXDS)
				#CALL EXF(1,ALEFT,Data.obj_cdata.USUPE,CMINDS,CMAXDS, EFFDSC,DEXDAR)
				QSUP = CMINDS*EFFDSC*(TS3 - TDEW)
				TAIR = TAIR + QSUP/CAIR
