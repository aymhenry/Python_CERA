...
class BData:
    ...
    # array(Rows, Cols) = [[0] * Cols for i in range(Rows)]
    C = [[0.0] * (2 + 1) for i in range(5 + 1)]

    # -- Common RDATA1 group ---------------------------------
    # Create zero base array
    # array(Rows, Cols) = [[0] * Cols for i in range(Rows)]
    A = [[0.0] * (2 + 1) for i in range(5 + 1)]
    # array(Rows, Cols) = [[0] * Cols for i in range(Rows)]
    B = [[0.0] * (2 + 1) for i in range(5 + 1)]
    ...
    def setup():
        ...
        # error last item fixed (was 0.181)
        BData.CRIT.append([120.91, 243.39, 384.95, 4180.0, 0.241])
        ...
        BData.HZERO[26] = -8695.95
        # DATA HZERO(26),SZERO(26)  -8695.95, 170.53 ] )
        BData.SZERO[26] = 170.53
...
class Block2 (BData):
    ...
    def bconst(self, NCIN, IR, FIN):
        ...
        for KR in range(1, BData.NC + 1):			# DO 100 KR = 1,NC
            ...
            # WM(KR) = CRIT(1,IR(KR))
            BData.WM[KR] = BData.CRIT[IR[KR] - 1][1 - 1]
            ...
            for KC in range(0, 2 + 1):  # DO 100 KC = 0,2
                ...
                BData.A[KC][KR - 1] = BData.COEFF[IR[KR] - 1][KC + 1 - 1]
                # B(KC,KR) = COEFF(KC + 4,IR(KR))
                BData.B[KC][KR - 1] = BData.COEFF[IR[KR] - 1][KC + 4 - 1]
                # C(KC,KR) = COEFF(KC + 7,IR(KR))
                BData.C[KC][KR - 1] = BData.COEFF[IR[KR] - 1][KC + 7 - 1]
        ...
        for KR in range(1, BData.NC + 1):  # DO 164 KR = 1,NC
            ...
            [X, XV, P, BData.VR[KR], VV, _] = self.bublt(
                BData.TREF[KR], X, XV, True)

            #[P5, P6, P7, P8] = self.hcvcps (P1, P2, P3, P4)
            # CALL HCVCPS (1,TREF(KR),VR(KR),X,   HRKR,CV,CPX,VS)
            [HRKR, CV, CPX, VS] = self.hcvcps(
                1, BData.TREF[KR], BData.VR[KR], X)

            # HR(KR) = HRKR - HZERO(IR(KR))
            BData.HR[KR] = HRKR - BData.HZERO[IR[KR]]
            BData.SR[KR] = self.entrop(
                BData.TREF[KR], BData.VR[KR], X) - BData.SZERO[IR[KR]]
        return
    ...
    def espar(self, IQ, T, X):  # ,AMIX,BMIX
        ...
        # Local Vars
        # array(Rows, Cols) = [[0] * Cols for i in range(Rows)]
        AJI = [[0.0] * (5 + 1) for i in range(5 + 1)]
        # array(Rows, Cols) = [[0] * Cols for i in range(Rows)]
        DA = [[0.0] * (5 + 1) for i in range(5 + 1)]
        # array(Rows, Cols) = [[0] * Cols for i in range(Rows)]
        DAJI = [[0.0] * (5 + 1) for i in range(5 + 1)]
        AMIX = 0.0
        BMIX = 0.0

        for I in range(1, BData.NC + 1):  # DO 120 I = 1,NC
            BData.AP[I] = BData.A[0][I - 1] * \
                math.exp((BData.A[1][I - 1] + BData.A[2][I - 1] * T) * T)
            BData.BP[I] = BData.B[0][I - 1] + \
                (BData.B[1][I - 1] + BData.B[2][I - 1] * T) * T
            ...
            for J in range(1, (I - 1) + 1):  # DO 120 J  =  1,I - 1
                AJI[J][I] = X[J] * X[I] * \
                    (1.0 - BData.F[J][I]) * math.sqrt(BData.AP[J] * BData.AP[I])
        ...
        if IQ >= 1:
            ...
            for I in range(1, BData.NC + 1):  # DO 140 I  =  1,NC
                ...
                BData.DBDT = BData.DBDT + \
                    X[I] * (BData.B[1][I - 1] + 2.0 * BData.B[2][I - 1] * T)

                for J in range(1, (I - 1) + 1):  # DO 140 J  =  1,I - 1
                    DA[J][I] = 0.5 * (BData.A[1][J - 1] + BData.A[1][I - 1]) + \
                        (BData.A[2][J - 1] + BData.A[2][I - 1]) * T
            ...
            if IQ >= 2:
                ...
                for I in range(1, BData.NC + 1):  # DO 160 I  =  1,NC
                    BData.CP[I] = BData.C[0][I - 1] + \
                        (BData.C[1][I - 1] + BData.C[2][I - 1] * T) * T

                    BData.D2BDT = BData.D2BDT + 2.0 * X[I] * BData.B[2][I - 1]
                    BData.D2ADT = BData.D2ADT + \
                        DAJI[I][I] * DA[I][I] + 2.0 * AJI[I][I] * BData.A[2][I - 1]

                    for J in range(1, (I - 1) + 1):  # DO 160 J  =  1,I - 1
                        BData.D2ADT = BData.D2ADT + 2.0 * \
                            (DAJI[J][I] * DA[J][I] + AJI[J][I] * (BData.A[2][J - 1] + BData.A[2][I - 1]))
            ...
            if IQ <= 3:
                ...
                for I in range(1, BData.NC + 1):  # DO 180 I  =  1,NC
                    ...
                    BData.HP[I] = (BData.C[0][I - 1] + (0.5 * BData.C[1]
                                                        [I - 1] + BData.C[2][I - 1] / 3.0 * T) * T) * T
    ...
    def gibbs(T, V, A, B):
        ...
        '''
        return BData.R * T * (- math.log(V) + 0.25 * B * ((8.0 * V - 2.25 * B) * V + 0.1875 * B * B) / (
            (V - 0.25 * B)**2) / (V - 0.25 * B)) + A / B * math.log(V / (V + B)) - A / (V + B)
    ...
    def bublt(self, T, XL, XV, LBUB):
        ...
        if T > 0.99 * loc_TC:
            LCRIT = True
            print(
                'bublt : critical point of pure or pseudo - pure material exceeded in bublt')
            print(
                '      in python, bad entry data, input temperature is more than critical point')
            print('      input temp=' +
                  str(T) +
                  "K Critical point (with the giiven X) " +
                  str(.099 *
                      loc_TC) +
                  " K")
            print('      input X ', X1)
        ...
        for IT in range(1, BData.ITMAX + 1):  # DO 400 IT = 1,ITMAX
            ...
            [VL, LV1CON] = self.vit(T, PP[J], A1, B1, VL, True)
            # CALL VIT (T, PP[J], A1, B1, VV, False, LV2CON)
            [VV, LV2CON] = self.vit(T, PP[J], A1, B1, VV, False)
        ...
        if BData.NC == 1:  # 440 if (BData.NC == 1)  :
            if LV1CON:
                print(
                    BData.LUP,
                    'bublt :volume iteration for parent phase did not converge')
            if LV2CON:
                print(
                    BData.LUP,
                    'bublt :volume iteration for incipient phase did not converge')
            if LPPCON:
                print(
                    BData.LUP,
                    'bublt :pure material pressure iteration in bublt did not converge')
            P = PP[J]
        ...
        for ITP in range(1, BData.ITMAX + 1):  # DO 800 ITP = 1,ITMAX
            ...
            [V1, LV1CON] = self.vit(T, PP[J], A1, B1, V1, LBUB)
            ...
            for I in range(1, BData.NC + 1):  # DO 540 I = 1,BData.NC
                U1[I] = self.U_Func(
                    T, X1, I, V1, A1, B1, BData.AP, BData.BP, BData.F)
            ...
            for IT in range(1, BData.ITMAX + 1):  # DO 600 IT = 1,ITMAX
                ...
                [A2, B2] = self.espar(0, T, XX2)
                #[P5, P7] = self.vit (P1, P2, P3, P4, P5, P6)
                # CALL VIT (T,PP[J], A2, B2,V2,.NOT.LBUB,LV2CON)
                [V2, LV2CON] = self.vit(T, PP[J], A2, B2, V2, not LBUB)
                ...
                for I in range(1, BData.NC + 1):  # DO 560 I = 1,BData.NC
                    U2[I] = self.U_Func(
                        T, XX2, I, V2, A2, B2, BData.AP, BData.BP, BData.F)
        ...
        if not b_pyth_exit_inner:
            print(
                BData.LUP,
                'bublt : mixture pressure iteration in bublt did not converge')
        ...
        if LXCON:
            print(
                BData.LUP,
                'bublt : composition iteration in bublt did not converge')
    ...
    def bublp(self, P, XL, XV, LBUB)	:  # T,VL,VV, LCRIT
        ...
        if (BData.NC == 1):
            ...
            for IT in range(1, BData.ITMAX + 1):   # DO 400 IT = 1,ITMAX
                ...
                [VL, LV1CON] = self.vit(TT[J], P, A1, B1, VL, True)
                # CALL VIT (TT(J), P, A1,B1, VV, False, LV2CON)
                [VV, LV2CON] = self.vit(TT[J], P, A1, B1, VV, False)
                ...
                if (IT <= 1):
                    ...
                    # PREVIOUS BEST GUESS, USE REGULI - FALSI METHOD FOR NEXT
                    # GUESS
                    if (LTNEG and LTPOS):
                        if (TT[3] < min(BData.TNEG, BData.TPOS)
                                or TT[3] > max(BData.TNEG, BData.TPOS)):
                            TT[3] = BData.TPOS - FTPOS * \
                                (BData.TPOS - BData.TNEG) / (FTPOS - FTNEG)
            ...
            if (LV1CON):
                print(
                    'bublp: iteration in bublp for parent phase volume, did not converge')
            if (LV2CON):
                print(
                    'bublp: iteration in bublp for incipient phase volume, did not converge')
            if (LPPCON):
                print(
                    'bublp: pure material temperature iteration in bublp, did not converge')
            T = TT[J]
        ...
        for ITT in range(1, BData.ITMAX + 1):   # DO 800 ITT = 1,ITMAX
            ...
            [V1, LV1CON] = self.vit(TT[J], P, A1, B1, V1, LBUB)
            #
            ...
            for I in range(1, BData.NC + 1):   # DO 540 I = 1,BData.NC
                U1[I] = self.U_Func(
                    TT[J], X1, I, V1, A1, B1, BData.AP, BData.BP, BData.F)
            #
            #    ENTER INNER ITERATION LOOP (FOR COMPOSITION OF PHASE 2)
            #
            ...
            for IT in range(1, BData.ITMAX + 1):   # DO 600 IT = 1,ITMAX
                ...
                #    COMPUTE EQUATION OF STATE COEFFICIENTS FOR PHASE 2
                # [P4, P5] = self.espar [P1, P2, P3]
                # JJ allway 1	CALL espar (0,TT(J),XX2(JJ), A2, B2)
                [A2, B2] = self.espar(0, TT[J], XX2)
                #[P5, P7] = self.vit (P1, P2, P3, P4, P5, P6)
                # CALL VIT (TT(J),P, A2, B2, V2,.NOT.LBUB, LV2CON)
                [V2, LV2CON] = self.vit(TT[J], P, A2, B2, V2, not LBUB)
                ...
                for I in range(1, BData.NC + 1):  # DO 560 I = 1,BData.NC
                    U2[I] = self.U_Func(
                        TT[J], XX2, I, V2, A2, B2, BData.AP, BData.BP, BData.F)
                #
                #    CALCULATE THE COMPOSITION OF PHASE 2 FROM THE COMPOSITION
                #    OF PHASE 1 AND THE CHEMICAL POTENTIALS.  THE INNER ITERATION
                #    LOOP HAS CONVERGED WHEN THE CALCULATED COMPOSITION EQUALS
            ...
            if not b_python_flag_loop3:
                print(IT, FXSUM, BData.NC * BData.TOLR,
                      FXSUM / (BData.NC * BData.TOLR), FXDIF)
                LXCON = True
            #
            #    END OF ITERATION LOOP FOR PHASE 2 COMPOSITION
            ...
            if (ITT <= 2):
                ...
                TT[3] = 1.0 / (1.0 / TT[J] + 0.0004 *
                               (math.log(P2) - math.log(P)))
                ...
                # FOR THIRD AND SUBSEQUENT ITERATIONS, USE SECANT / REGULI -
                # FALSI
                TT[3] = TT[2] - FT[2] * (TT[2] - TT[1]) / (FT[2] - FT[1])

                if ((TT[3] > max(BData.TNEG, BData.TPOS) or TT[3] <
                     min(BData.TNEG, BData.TPOS)) and LTNEG and LTPOS):
        ...
        if not b_python_flag_loop8:
            print(
                BData.LUP,
                'MIXTURE TEMPERATURE ITERATION IN BUBLP DID NOT, CONVERGE')
        ...
        if (LV1CON):
            print(BData.LUP, 'ITERATION IN BUBLP FOR PARENT PHASE VOLUME DID')
        if (LV2CON):
            print(
                BData.LUP,
                'ITERATION IN BUBLP FOR INCIPIENT PHASE VOLUME DID, NOT CONVERGE')
        if (LXCON):
            print(BData.LUP, 'COMPOSITION ITERATION IN BUBLP DID NOT CONVERGE')
    ...
    def entrop(self, T, V, X):
        ...
        # similar to COMMON /ESPAR1/ AP(5),BP(5),F(5,5),
        # DADT,DBDT,D2ADT,D2BDT
        ...
        for I in range(1, BData.NC + 1):  # DO 120 I=1,NC
            S = S + X[I] * (BData.SP[I] - BData.SR[I] +
                            BData.R * math.log(V / BData.VR[I]))
    ...
    def hcvcps(self, IQ, T, V, X):  # H, CV, CP, VS
        ...
        if (IQ <= 3):
            ...
            H = ((A + (A * BData.DBDT / B - BData.DADT) * T) * VBL + A * (BData.DBDT * T - B) /
                 VB) / B + 2.0 * RT * V * (2.0 * V - B4) * (B4 - 0.25 * BData.DBDT * T) / VB43
        ...
        if (IQ >= 2):
            ...
            if (IQ == 3 or IQ == 5):
                ...
                DPDV = (- RT * (1.0 + (4.0 + (4.0 + (- 4.0 + Y) * Y) * Y) * Y) /
                        ((1.0 - Y)**4) + A * (2.0 * V + B) / (VB * VB)) / (V * V)

                CP = CV - DPDT * DPDT * T / DPDV

                # COMPUTE VELOCITY OF SOUND USING C'S AND VOLUME DERIVATIVE OF
                # P
                WMOL = 0.0
    ...
    def vit(self, T, P, para_A, para_B, VS, LLIQI):  # VS, LVCON
        ...
        for IT in range(1, BData.ITMAX + 1):   # DO 100 IT = 1,ITmath.max
            ...
            #
            P2 = (RT * (1.0 + (1.0 + (1.0 - Y) * Y) * Y) /
                  (1.0 - Y)**3 - para_A / VB) / V
            DPDLV = RT / V * (- 1.0 + (- 4.0 + (- 4.0 + (4.0 - Y) * Y) * Y) * Y) / \
                (pow(1.0 - Y, 4)) + para_A * (2.0 * V + para_B) / (V * VB * VB)
    ...
    def spxsp(self, S, P, X, TSAT, SSAT, VSAT, LLIQ):  # ,T2,V,LLIQ
        ...
        [HSAT, CV, CP, VSND] = self.hcvcps(4, T1, VSAT, X)
        ...
        for IT in range(1, BData.ITMAX + 1):   # DO 200 IT = 1,ITmath.max
            ...
            [V, LVCON] = self.vit(T2, P, A, B, V, LLIQ)
    ...
    def hpxsp(self, H, P, X, TSAT, HSAT, VSAT, LLIQ):  # T2,V, LLIQ
        ...
        [HSAT, CV, CP, VSND] = self.hcvcps(5, T1, VSAT, X)
        ...
        for IT in range(1, BData.ITMAX + 1):  # DO 200 IT=1,ITMAX
            ...
            [HSP, CV, CP, VSND] = self.hcvcps(1, T2, V, X)
    ...
    def spin(self, S, P, X):  # T,XQ,XL,XV,VL,VV,SL,SV
        ...
        [_, XV, TBUB, VL, VVBUB, LCRIT] = self.bublp(P, X, XV, True)
        # CALL BUBLP (P,XL,X,    TDEW,VLDEW,VV,   False ,LCRIT)
        [XL, _, TDEW, VLDEW, VV, LCRIT] = self.bublp(P, XL, X, False)
        ...
        if (S <= SL):
            ...
            b_python_flag = False
            for IT in range(
                    1, int(BData.ITMAX / 2) + 1):  # DO 260 IT = 1,ITMAX / 2
                #
                ...
                if (IT <= 1 or NCC <= 2):
                    ...
                    for J in range(
                            1, (BData.NC - 1) + 1):  		# DO 222 J = 1,BData.NC - 1
                        for K in range(
                                J + 1, BData.NC + 1):		# DO 222 K = J + 1,BData.NC
                ...
                [_, XVB, TBUB, VLBUB, VVBUB, LCRIT] = self.bublp(
                    P, XLB, XVB, True)
                # CALL BUBLP (P,XLD,XVD,   TDEW,  VLDEW,VVDEW,  False ,LCRIT)
                [XLD, _, TDEW, VLDEW, VVDEW, LCRIT] = self.bublp(
                    P, XLD, XVD, False)
            ...
            if not b_python_flag:
                print(BData.LIP, "ROUTINE SPIN DID NOT CONVERGE S, P, X",
                      S, P, X[1:BData.NC])
    ...
    def hpin(self, H, P, X):
        ...
        # CALL BUBLP (P,X,XV  ,TBUB, VL,VVBUB,  .TRUE.,LCRIT)
        [X, XV, TBUB, VL, VVBUB, LCRIT] = self.bublp(P, X, XV, True)
        # CALL BUBLP (P,XL,X,  TDEW,VLDEW,VV,.FALSE.,LCRIT)
        [XL, X, TDEW, VLDEW, VV, LCRIT] = self.bublp(P, XL, X, False)

        VLBUB = VL  # in Python only

        #[P5, P6, P7, P8] = self.hcvcps (P1, P2, P3, P4)
        # CALL HCVCPS (1,TBUB,VL,X, HL,CV,CP,VSND)
        [HL, CV, CP, VSND] = self.hcvcps(1, TBUB, VL, X)
        # CALL HCVCPS (1,TDEW,VV,X ,HV,CV,CP,VSND)
        [HV, CV, CP, VSND] = self.hcvcps(1, TDEW, VV, X)
        ...
        if (H <= HL):
            ...
            [T, VL] = self.hpxsp(H, P, X, TBUB, HL, VLBUB, True)
            ...
            [T, VV] = self.hpxsp(H, P, X, TDEW, HV, VVDEW, False)
            ...
            [HVB, CV, CP, VSND] = self.hcvcps(1, TBUB, VVBUB, XVB)
            # CALL HCVCPS (1,TDEW,VLDEW,XLD  ,HLD,CV,CP,VSND)
            [HLD, CV, CP, VSND] = self.hcvcps(1, TDEW, VLDEW, XLD)
            ...
            for IT in range(
                ...
                if (abs(XQCB - XQHB) > 0.0001):
                    LCONV = False
                if (abs(XQCD - XQHD) > 0.0001):
                    LCONV = False
                ...
                if (IT < 1 or NCC < 2):
                    ...
                    for J in range(1, (BData.NC - 1) + 1):  # DO 222 J=1,NC-1
                        for K in range(
                                J + 1, (BData.NC + 1)):  # DO 222 K=J+1,NC
                            DTSUMB = DTSUMB + ((X[J] - XLB[J]) * (XVB[K] - XLB[K])
                                               - (X[K] - XLB[K]) * (XVB[J] - XLB[J]))**2
                ...
                # [P2, P3, P4, P5, P6, P8] = bublp ( P1, P2, P3,    P7)
                # CALL BUBLP (P,XLB,XVB, TBUB,VLBUB,VVBUB,.TRUE.,LCRIT)
                [XLB, XVB, TBUB, VLBUB, VVBUB, LCRIT] = self.bublp(
                    P, XLB, XVB, True)
                # CALL BUBLP (P,XLD,XVD, TDEW,VLDEW,VVDEW, False,LCRIT)
                [XLD, XVD, TDEW, VLDEW, VVDEW, LCRIT] = self.bublp(
                    P, XLD, XVD, False)

                #[P5, P6, P7, P8] = self.hcvcps (P1, P2, P3, P4)
                # CALL HCVCPS (1,TBUB,VLBUB,XLB,  HLB,CV,CP,VSND)
                [HLB, CV, CP, VSND] = self.hcvcps(1, TBUB, VLBUB, XLB)
                # CALL HCVCPS (1,TBUB,VVBUB,XVB  ,HVB,CV,CP,VSND)
                [HVB, CV, CP, VSND] = self.hcvcps(1, TBUB, VVBUB, XVB)
                # CALL HCVCPS (1,TDEW,VLDEW,XLD,  HLD,CV,CP,VSND)
                [HLD, CV, CP, VSND] = self.hcvcps(1, TDEW, VLDEW, XLD)
                # CALL HCVCPS (1,TDEW,VVDEW,XVD,  HVD,CV,CP,VSND)
                [HVD, CV, CP, VSND] = self.hcvcps(1, TDEW, VVDEW, XVD)
            ...
            if not b_python_flag:
                print(
                    BData.LUP,
                    'ROUTINE HPIN DID NOT CONVERGE; H,P,X:',
                    H,
                    P,
                    X)  # print array x
                # 1000 FORMAT (1X,'ROUTINE HPIN DID NOT CONVERGE;
                # H,P,X:',2F12.4,5F8.5)
    ...
    def U_Func(self, T, X, K, V, A, B, AP, BP, F):
        ...
        for I in range(1, BData.NC + 1):  # DO 120 I=1,NC
            ...
            if (I != K):
                DA = DA - 2.0 * X[I] * F[K][I] * SQAK * math.sqrt(AP[I])
            # 120 CONTINUE
