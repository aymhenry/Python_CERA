...
class ViewCycle:
	...
	def	 setBasicData (self):		
		...
		self.obj_parameter.HSTATE = ['','COMP IN','COMP DIS','COND IN','COND DEW',	\
					'COND BUB','COND OUT','LIQ LINE',	\
					'SUBCOOL1','SUBCOOL2','FREZ IN ','FREZ OUT','FRSH IN ',	\
					'FRSH DEW','FRSH OUT','HX1 OUT ']

		# in python add extra item for item 0
		self.obj_parameter.MSTATE = ['','COMP IN','COMP DIS','COND IN','COND DEW',	\
					'COND BUB','COND OUT','LIQ LINE',			\
					'SUBCOOL ','EVAP IN ', 'EVAP DEW','EVAP OUT','HX OUT  ']

		# in python add extra item for item 0
		self.obj_parameter.LPNT  = [0,1,2,14,3,11,4,16,6,10,8,9,5,12,7,13]					
	...
	def show_rep( self ):
		...
		if (self.obj_parameter.LQUIT):
			objCycOut.write_or_terminate ("Convergence forced after, iterations ", self.Data.IC)
		...
		if (self.obj_parameter.LECON  or  self.obj_parameter.LCCON  or  self.obj_parameter.I_ERROR_INTER  > 0) :
			...
			if (self.obj_parameter.LCCON) :
				objCycOut.write_or_terminate  ('CONDENSER ITERATION DID NOT CONVERGE,  %9.3f, %9.3f ' %( self.obj_parameter.TC[1]-273.11,self.obj_parameter.TC[2] -273.11) )
				self.showError ('CONDENSER ITERATION DID NOT CONVERGE ,  %9.3f, %9.3f ' %( self.obj_parameter.TC[1]-273.11,self.obj_parameter.TC[2]-273.11))
		...
		if (self.obj_parameter.ICYCL  ==  2) :
			while (K <= 15): #DO WHILE (K  <=  15)
				...
				if (self.obj_parameter.XQ[J] >  1.0): self.obj_parameter.XQ[J] = 1.0
		...
		if (self.Data.ITYPE  ==  1) :
			objCycOut.write_or_terminate  ('EVAPORATOR CAPACITY,  %9.3f, KJ/HR,  %9.3f, W' %( self.Data.QE, self.Data.QE/3.6) )
		else:
			objCycOut.write_or_terminate  ('FRESH FOOD EVAPORATOR CAPACITY,  %9.3f, KJ/HR,  %9.3f, W' %( self.Data.QE, self.Data.QE/3.6) )
			objCycOut.write_or_terminate  ('FREEZER EVAPORATOR CAPACITY,     %9.3f, KJ/HR,  %9.3f, W' %( self.Data.QZ, self.Data.QZ/3.6) )

		
		objCycOut.write_or_terminate  ('CONDENSER HEAT REJECTION RATE,  %9.3f, KJ/HR,  %9.3f, W' %( self.obj_parameter.QC, self.obj_parameter.QC/3.6) )
		objCycOut.write_or_terminate  ('COMPRESSOR POWER REQUIREMENT,     %9.3f, KJ/HR,  %9.3f, W' %(self.Data.W, self.Data.W/3.6 ) )
		objCycOut.write_or_terminate  ('COEFFICIENT OF PERFORMANCE,     %9.3f' %( self.Data.COPR) )

		if (self.obj_parameter.IRFTYP  <=  3  and  self.obj_parameter.ICYCL  ==  1  and  self.obj_parameter.ICAB  !=  0
		...
		if (self.Data.IMAP  ==  1) :
			objCycOut.write_or_terminate  ('ESTIMATED COMPRESSION EFFICIENCY, %9.3f, (COMPRESSOR EER MODEL)' %( self.obj_parameter.ETAS) )
			objCycOut.write_or_terminate  ('ESTIMATED MOTOR-PUMP EFFICIENCY,  %9.3f, (COMPRESSOR EER MODEL)' %( self.Data.EFFC/ self.obj_parameter.ETAS) )
			objCycOut.write_or_terminate  ('ESTIMATED CLEARANCE VOLUME,       %9.3f, (COMPRESSOR EER MODEL)' %( self.Data.CE) )
			objCycOut.write_or_terminate  ('ESTIMATED SHELL LOSS,       %9.3f, (COMPRESSOR EER MODEL)' %( self.obj_parameter.QCAN) )
			objCycOut.write_or_terminate  ('ESTIMATED DISC TUBE HEAT LOSS,       %9.3f, (COMPRESSOR EER MODEL)' %( self.obj_parameter.QHILO) )
		...
		if (self.Data.ITYPE  ==  1) :
			if (self.Data.IFRSH  !=  0) :
				objCycOut.write_or_terminate  ('EVAPORATOR,     %9.3f, N/A, ,%9.3f' %(self.Data.ETAE,self.obj_parameter.FSUPE ) )
			else:
				objCycOut.write_or_terminate  ('EVAPORATOR,     , N/A, N/A, %9.3f ' %(self.obj_parameter.FSUPE ) )
		else:
			if (self.Data.IFRSH  !=  0) :
				objCycOut.write_or_terminate  ('FRESH FOOD EVAP.,     %9.3f, N/A, %9.3f ' %(self.Data.ETAE,self.obj_parameter.FSUPE ) )
			else:
				objCycOut.write_or_terminate  ('FRESH FOOD EVAP.,  , N/A, N/A, %9.3f ' %(self.obj_parameter.FSUPE ) )
				
			if (self.Data.IFREZ  !=  0) :
				objCycOut.write_or_terminate  ('FREEZER EVAP.,   %9.3f, N/A,  ---- ' %( self.Data.ETAF ) )
			else:
				objCycOut.write_or_terminate  (' ' )


		if (self.Data.ICOND  !=  0) :
			objCycOut.write_or_terminate  ('CONDENSER,     %9.3f,%9.3f, %9.3f ' %(self.Data.ETAC,self.obj_parameter.FSUBC,self.obj_parameter.FSUPC ) )
		else:
			objCycOut.write_or_terminate  ('CONDENSER,     , N/A, %9.3f,%9.3f ' %(self.obj_parameter.FSUBC,self.obj_parameter.FSUPC ) )
		
		objCycOut.write_or_terminate ("COMPRESSOR PERFORMANCE SUMMARY")
		objCycOut.write_or_terminate ('REFRIGERANT MASS FLOW RATE.,   %9.3f, KG/HR' %( self.Data.FLOW * 0.45359 ) )
		
		if (self.Data.IMAP != 0): 
			objCycOut.write_or_terminate ('VOLUMETRIC EFFICIENCY,   %9.3f' %(  self.Data.ETAV * 0.45359 ) )

		objCycOut.write_or_terminate ('PRESSURE RATIO,   %9.3f' %( self.obj_parameter.PR ) )
		
		if (self.Data.IMAP != 0) :
			objCycOut.write_or_terminate ('SUCTION PORT TEMPERATURE (C),   %9.3f' %( self.obj_parameter.TSUC ) )
			objCycOut.write_or_terminate ('DISCHARGE PORT TEMPERATURE (C),   %9.3f' %( self.obj_parameter.TDISC ) )
			objCycOut.write_or_terminate ('DISCHARGE SUPERHEAT (C),   %9.3f' %( self.obj_parameter.TSUPC ) )
