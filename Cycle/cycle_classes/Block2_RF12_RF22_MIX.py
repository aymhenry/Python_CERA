...
def main ():
	...
	print ("	 USE RF12 & RF22 -- for tessting -=-= obj_block2.bconst (NC, IR, FIN)")
	code = 2
	code2 = 6
	NC = 2
	IR = [0.0, code, code2,0,0,0] 
	...
	MOLAR_WEIGHT1 = obj_block2.WM [1]
	MOLAR_WEIGHT2 = obj_block2.WM [2]
	MOLAR_WEIGHT = (MOLAR_WEIGHT1 + MOLAR_WEIGHT2)/2 # 50% for each one used
	
	print ("\n	 ---- Output --")
	print ("		many, but i need Molare weight g/mole      = ", MOLAR_WEIGHT1)
	print ("		                 MOLAR_WEIGHT2 g/mole      = ", MOLAR_WEIGHT2)
	
	#---------------------------------------------------------------
	Title ("	[A1, B1] = obj_block2.espar (IQ,T,X)")
	T=273+20
	...
	[A1, B1] = obj_block2.espar (IQ,T,X)	# 2 i.e. calc evry thing
	print ("\n	 ---- Output --")
	print ("		A1 = %5.3f"  % (A1) )
	print ("		B1 = %5.3f"  % (B1) )
	...
	R12_R22_TABLE_READING = 200.0- 34.871	# kj/kg   200-app result at 0C need to be 200, valid only for R12_R22
	R12_R22_TABLE_READING_S = 1 - 0.10939304044174783 	# kj/kg.K 1-app result at 0C need to be 1.0, valid only for R12_R22
	...
	print ("\n	 ---- Output ===========-")
	print ("		H in kJ/k-mole= %5.3f,  kJ/kg=%5.3f,  Reading in ASHRAE kJ/kg = %5.3f" %(H, H/MOLAR_WEIGHT, H/MOLAR_WEIGHT + R12_R22_TABLE_READING) )
	...
	print ("\n	 ---- Output --")
	print ("		S in kJ/k-mole K= %5.3f,  kJ/kg K=%5.3f,  Reading in ASHRAE kJ/kg K= %5.3f" %(S, S/MOLAR_WEIGHT, S/MOLAR_WEIGHT + R12_R22_TABLE_READING_S) )
	print ("		S=",S/MOLAR_WEIGHT)
	#---------------------------------------------------------------
	Title ("	S = obj_block2.entrop (T, VV_m3_per_kmole ,X)")
	...
	Happ =  (Hbook - R12_R22_TABLE_READING)*MOLAR_WEIGHT
	...
	print ("		HL in kJ/k-mole= %5.3f,  kJ/kg=%5.3f,  Reading in ASHRAE kJ/kg = %5.3f" %(HL, HL/MOLAR_WEIGHT, HL/MOLAR_WEIGHT + R12_R22_TABLE_READING) )
	print ("		HV in kJ/k-mole= %5.3f,  kJ/kg=%5.3f,  Reading in ASHRAE kJ/kg = %5.3f" %(HV, HV/MOLAR_WEIGHT, HV/MOLAR_WEIGHT + R12_R22_TABLE_READING) )

	#---------------------------------------------------------------
	Title ("	[T, XQ,XL,XV,  VL,VV,  HL,HV] = obj_block2.spin ( S,P,X )")
	Sbook =  1.168975  #related to XQ=.25%
	P = 448.95 # related to 12C
	S =  (Sbook - R12_R22_TABLE_READING_S)*MOLAR_WEIGHT
	...
	print ("		SL in kJ/k-mole K= %5.3f,  kJ/kg K=%5.3f,  Reading in ASHRAE kJ/kg K= %5.3f" %(SL, SL/MOLAR_WEIGHT, SL/MOLAR_WEIGHT + R12_R22_TABLE_READING_S) )
	print ("		SV in kJ/k-mole K= %5.3f,  kJ/kg K=%5.3f,  Reading in ASHRAE kJ/kg K= %5.3f" %(SV, SV/MOLAR_WEIGHT, SV/MOLAR_WEIGHT + R12_R22_TABLE_READING_S) )
...
def bublt_test():
	...
	R12_R22_TABLE_READING = 200.0- 26.2257538946007

	Hbook =  370
	P = 400
	Happ =  (Hbook - R12_R22_TABLE_READING)*120.91
	#print ("  h  =", h, " in kj/kg", h/120.91, " reading in table ",  h/120.91 + R12_R22_TABLE_READING)
	print ("		P=", P)
	print ("		Happ=", Happ)
	print ("		Hbook=", Happ/120.91 + R12_R22_TABLE_READING)
	...
	print ("	book m3/kg K	HL=",  HL/120.91 + R12_R22_TABLE_READING)
	
	print ("\n		HV=", HV)	
	print ("	m3/kg K	HV=", HV/120.91)
	print ("	book m3/kg K	HV=",  HV/120.91 + R12_R22_TABLE_READING)
