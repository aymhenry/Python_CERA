...
class View (ABC):
	...
	def rep_title (self):
		...
		self.objCabout.write_or_terminate( ( "RUN AT " + now.strftime( "%H %M %S %d %b %Y" ) ) + " - Python Output aymhenry@gmail")
	...
	def sec04_Mode3(self):
		self.objCabout.write_or_terminate (" ")
		self.objCabout.write_or_terminate ("Mullion")
		self.objCabout.write_or_terminate (" ,Top of the mullion, % 7.3f, CM, from the top of the cabinet" %( Unit.feet_cm(self.obj_data.TOPMUL)) )
		self.objCabout.write_or_terminate (" ,Mullion thickness,% 7.3f, CM" %( Unit.feet_cm(self.obj_data.THMUL)) )
	...
	def sec13_Mode3(self):
		...
		self.objCabout.write_or_terminate ("  ,Freezer,% 10.4f, W/m-C"      %( Unit.BtuThHInchF_WattM (self.obj_data.HLFZG)  ) )
		self.objCabout.write_or_terminate ("  ,Refrigerator,% 10.4f, W/m-C" %( Unit.BtuThHInchF_WattM (self.obj_data.HLRG )  ) )
	...
	def sec14_Mode28(self):
		self.objCabout.write_or_terminate (" ")
		self.objCabout.write_or_terminate ("Gasket Heat Leak")
		self.objCabout.write_or_terminate ("  ,Freezer, % 10.4f, W/m-C"     %( Unit.BtuThHInchF_WattM (self.obj_data.HLGZF) ) )
		self.objCabout.write_or_terminate ("  ,Refrigerator,% 10.4f, W/m-C" %( Unit.BtuThHInchF_WattM (self.obj_data.HLRG)  ) )
	...
	def final_Mode57(self):	
		...
		self.objCabout.write_or_terminate (' ,Right wall'   + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QRSIDE) ))
		self.objCabout.write_or_terminate (' ,Left wall'    + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QLSIDE) ))
		self.objCabout.write_or_terminate (' ,Back wall'    + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QBACK)  ))
		self.objCabout.write_or_terminate (' ,Front (door)' + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QFRONT) ))
		self.objCabout.write_or_terminate (' ,Top (door)'   + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QTOP)   ))
		self.objCabout.write_or_terminate (' ,Bottom'       + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QBOTTM) ))

		self.objCabout.write_or_terminate (' ,Wedge'        + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QW)   ))
		self.objCabout.write_or_terminate (' ,Gasket'       + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QGZF) ))

		self.objCabout.write_or_terminate (' ,Open door sensible load'  + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QSDRFZ)  ))
		self.objCabout.write_or_terminate (' ,Moisture load - condense' + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QLDRFZ)  ))
		self.objCabout.write_or_terminate (' ,Moisture load - frost'    + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QFDRFZ)  ))

		self.objCabout.write_or_terminate (' ,Anti-sweat heater'    + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.FZASHQ)  ))
		self.objCabout.write_or_terminate (' ,Refrigerant line heat'+ ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.FZREFQ)  ))
		self.objCabout.write_or_terminate (' ,Penetrations'         + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.FZPENA)  ))
		self.objCabout.write_or_terminate (' ,Other thermal input'  + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.FZHEAT)  ))
		self.objCabout.write_or_terminate ('Total ,'               + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QFZTOT)  ))
	...
	def sec1_cycle_Mode238 (self):
		...
		self.objCycout.write_or_terminate ("%10.2f,W ,Fresh food antisweat heater" % ( self.obj_data.FFASHW ) )
		self.objCycout.write_or_terminate ("%10.2f,W ,Fresh food auxiliary power" % ( self.obj_data.FFAUXW ) )
	...
	def sec4_cycle_Mode238(self):
		...
		self.objCycout.write_or_terminate ('%10.2f ,Fresh food net load'    % ( Unit.BtuH_Watt(self.obj_data.QFFTOT) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer load'           % ( Unit.BtuH_Watt(self.obj_data.QFZTOT) ))

		self.objCycout.write_or_terminate ('%10.2f ,Fresh food door sensible load'       % ( Unit.BtuH_Watt(self.obj_data.QSDRFF) ))
		self.objCycout.write_or_terminate ('%10.2f ,Fresh food door condensation load'   % ( Unit.BtuH_Watt(self.obj_data.QLDRFF) ))
		self.objCycout.write_or_terminate ('%10.2f ,Fresh food door frost load'          % ( Unit.BtuH_Watt(self.obj_data.QFDRFF) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer door sensible load'          % ( Unit.BtuH_Watt(self.obj_data.QSDRFZ) ))

		self.objCycout.write_or_terminate ('%10.2f ,Freezer door condensation load'   % ( Unit.BtuH_Watt(self.obj_data.QLDRFZ) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer door frost load'          % ( Unit.BtuH_Watt(self.obj_data.QFDRFZ) ))

		self.objCycout.write_or_terminate ('%10.2f ,Fresh food penetrations'          % ( Unit.BtuH_Watt(self.obj_data.FFPENA) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer penetrations'             % ( Unit.BtuH_Watt(self.obj_data.FZPENA) ))

		self.objCycout.write_or_terminate ('%10.2f ,Fresh food heaters and controls'  % ( Unit.BtuH_Watt(self.obj_data.QHTFF) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer heaters and controls'     % ( Unit.BtuH_Watt(self.obj_data.QHTFZ) ))

		self.objCycout.write_or_terminate ('%10.2f ,Fresh food refrigerant line'   % ( Unit.BtuH_Watt(self.obj_data.FFREFQ) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer refrigerant line'      % ( Unit.BtuH_Watt(self.obj_data.FZREFQ) ))

		self.objCycout.write_or_terminate ('%10.2f ,Mullion heat load'             % (   Unit.BtuH_Watt(-1*self.obj_data.QMULN) ))
	...
	def sec5_cycle_Mode457(self):
		...
		self.objCycout.write_or_terminate ('%10.2f ,Fresh food net load'   % ( Unit.BtuH_Watt(self.obj_data.QFZTOT) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer load'          % ( Unit.BtuH_Watt(self.obj_data.QFZTOT) ))

		self.objCycout.write_or_terminate ('%10.2f ,Fresh food door sensible load'       % ( Unit.BtuH_Watt(self.obj_data.QSDRFZ) ))
		self.objCycout.write_or_terminate ('%10.2f ,Fresh food door condensation load'   % ( Unit.BtuH_Watt(self.obj_data.QLDRFZ) ))
		self.objCycout.write_or_terminate ('%10.2f ,Fresh food door frost load'          % ( Unit.BtuH_Watt(self.obj_data.QFDRFZ) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer door sensible load'          % ( Unit.BtuH_Watt(self.obj_data.QSDRFZ) ))

		self.objCycout.write_or_terminate ('%10.2f ,Freezer door condensation load'    % ( Unit.BtuH_Watt(self.obj_data.QLDRFZ) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer door frost load'           % ( Unit.BtuH_Watt(self.obj_data.QFDRFZ) ))

		self.objCycout.write_or_terminate ('%10.2f ,Fresh food penetrations'           % ( Unit.BtuH_Watt(self.obj_data.FZPENA) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer penetrations'              % ( Unit.BtuH_Watt(self.obj_data.FZPENA) ))

		self.objCycout.write_or_terminate ('%10.2f ,Fresh food heaters and controls'   % ( Unit.BtuH_Watt(self.obj_data.QHTFZ) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer heaters and controls'      % ( Unit.BtuH_Watt(self.obj_data.QHTFZ) ))

		self.objCycout.write_or_terminate ('%10.2f ,Fresh food refrigerant line'  % ( Unit.BtuH_Watt(self.obj_data.FZREFQ) ))
		self.objCycout.write_or_terminate ('%10.2f ,Freezer refrigerant line'     % ( Unit.BtuH_Watt(self.obj_data.FZREFQ) ))

		self.objCycout.write_or_terminate ('%10.2f ,Mullion heat load'             % ( 0.0 ))	
	#-----------------------------------------------------------
...
class View_Ql2(View):
	def show_final_results (self):
		...
		self.objCabout.write_or_terminate (' ,Right wall'   + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(0.0),                  Unit.BtuH_Watt(self.obj_data.QRSIDE) ))
		self.objCabout.write_or_terminate (' ,Left wall'    + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QLSIDE), Unit.BtuH_Watt(0.0)       ))
		self.objCabout.write_or_terminate (' ,Back wall'    + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QBACKL), Unit.BtuH_Watt(self.obj_data.QBACKR) ))
		self.objCabout.write_or_terminate (' ,Front (door)' + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QFRNTL), Unit.BtuH_Watt(self.obj_data.QFRNTR) ))

		self.objCabout.write_or_terminate (' ,Top wall' + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QLTOP),  Unit.BtuH_Watt(self.obj_data.QRTOP ) ))
		self.objCabout.write_or_terminate (' ,Bottom'   + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QLBTTM), Unit.BtuH_Watt(self.obj_data.QRBTTM) ))
		...
		self.objCabout.write_or_terminate (' ,Penetrations'          + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.FFPENA), Unit.BtuH_Watt(self.obj_data.FZPENA)     ))
		self.objCabout.write_or_terminate (' ,Other thermal input'   + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.FFHEAT), Unit.BtuH_Watt(self.obj_data.FZHEAT) ))

		self.objCabout.write_or_terminate ('Total ,'                + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QFFTOT),   Unit.BtuH_Watt(self.obj_data.QFZTOT)     ))
...
class View_Ql13 (View):
	def show_final_results (self):
		...
		self.objCabout.write_or_terminate (' ,Right wall'   + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QRRSID),  Unit.BtuH_Watt(self.obj_data.QFRSID) ))
		self.objCabout.write_or_terminate (' ,Left wall'    + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QRLSID),  Unit.BtuH_Watt(self.obj_data.QFLSID) ))
		self.objCabout.write_or_terminate (' ,Back wall'    + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QRBACK),  Unit.BtuH_Watt(self.obj_data.QFBACK) ))
		self.objCabout.write_or_terminate (' ,Front (door)' + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QRFRNT),  Unit.BtuH_Watt(self.obj_data.QFFRNT) ))

		self.objCabout.write_or_terminate (' ,Top wall' + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(0.0),                   Unit.BtuH_Watt(self.obj_data.QTOP)   ))
		self.objCabout.write_or_terminate (' ,Bottom'   + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QBOTTM),  Unit.BtuH_Watt(0.0)       ))
		self.objCabout.write_or_terminate (' ,Mullion'  + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QMULN),   Unit.BtuH_Watt(self.obj_data.QMULI)  ))
		self.objCabout.write_or_terminate (' ,Wedge'    + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QWFF),    Unit.BtuH_Watt(self.obj_data.QWFZ)   ))
...
class View_Ql5(View):
	def show_final_results (self):
		...
		self.objCabout.write_or_terminate (' ,Top (door)'   + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QTOP)   ))
		self.objCabout.write_or_terminate (' ,Bottom'       + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QBTTM)      ))
		self.objCabout.write_or_terminate (' ,Gasket'       + ',%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QGZF)  ))
...
class View_Ql8(View):
	def show_final_results (self):
		...
		self.objCabout.write_or_terminate (' ,Top (door)'   + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(self.obj_data.QTOP),    Unit.BtuH_Watt(0.0)      )) # chk order
		self.objCabout.write_or_terminate (' ,Bottom'       + ',%10.2f,%10.2f'  % ( Unit.BtuH_Watt(0.0),                   Unit.BtuH_Watt(self.obj_data.QBOTTM)))
