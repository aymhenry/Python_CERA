...
class Ql8 (CabUtils):
	...
	def __init__(self, Cab):
		...
		if Cab.NCCTYPE == 2:
			AOFSID = (Cab.HEIGHT - Cab.TOPMUL - Cab.THMUL / 2.0) * (Cab.DEPTH - Cab.WEDGE - Cab.DGSKT) - Cab.CDDN * Cab.CCHGT / 2.0
		...
		#
		# The corner shape factor is divided by three since each corner is shared by three walls of different thicknesses.
		...
		loc_list = self.radtrn (R1, R2, Cab.TFF, Cab.TROOM)
		...
		loc_list = self.radtrn (R1, R2, Cab.TFRZ, Cab.TROOM)
		...
		RR = AIRSID / Cab.TIRRS + 0.54 * (DFFC + 2.0 * HFFC) / 2.0	\
			 + 0.15 * (2.0 * Cab.TIRRS + Cab.TIRB + 2.0 * Cab.TIRT + Cab.TIRF) / 9.
		R1 = 1.0 / (RR * Cab.RKINFF) + 1.0 / (Cab.HI * AIRSID)
		R2 = 1.0 / (Cab.HO * AORSID)
		...
		if Cab.NCCTYPE == 2:
			RF = AIFSID / Cab.TIFRS + 0.54 * (H1 + H2 + D1 + D2) / 2.0 + 0.15	\
				 * ((2.0 * Cab.BINSUL + Cab.TIFRS) * FALPHA	\
				 + (Cab.TIFB + Cab.TIFRS + Cab.BINSUL) * FBETA	\
				 + (Cab.TIFF + Cab.TIFRS + Cab.BINSUL)) / 9.0

		if Cab.NCCTYPE == 3:
			RF = AIFSID / Cab.TIFRS + 0.54 * (H1 + H2 + D1 + D2 + D3) / 2.0 + 0.15	\
				 * (2.0 * (2.0 * Cab.BINSUL + Cab.TIFRS) * FALPHA		\
		...
		#
		Cab.R = AIRTOP / Cab.TIRT + 0.54 * (2.0 * DFFC + 2.0 * WFFC) / 2.0 \
		...
		#
		if Cab.NCCTYPE == 1:
			Cab.R = AIFBCK / Cab.TIFB + 0.54 * (2.0 * HFRZ + WFRZ) / 2.0	\
		...
		if Cab.NCCTYPE == 1:
			...
			loc_list = self.radtrn (R1, R2, Cab.TFRZ, Cab.TBTM)
		...
		if Cab.WEDGER != 0.0:
			...
			Cab.QWFF = (1.0 / (1.0 / (Cab.HO * Cab.WEDGER * (2.0 * Cab.TOPMUL + Cab.WIDTH)) + THETA / (Cab.WKINR \
				 * math.log(AWEDGE / BWEDGE) * (WL1 + WL2) / 2.0))) * (Cab.TROOM - Cab.TFF)
		...
		QWFZS = (1.0 / (1.0 / (2.0 * Cab.HO * HFRZ * Cab.WEDGE) + THETA / (Cab.WKIN * \
			math.log(AWEDGE / BWEDGE) * (WL + HFRZ)))) * (Cab.TROOM - Cab.TFRZ)
		...
		QWFZB = (1.0 / (1.0 / (Cab.HO * Cab.WEDGE * Cab.WIDTH) + THETA / (Cab.WKIN * math.log(AWEDGE / BWEDGE) * (WL + WFRZ) * 0.5))) * (Cab.TBTM - Cab.TFRZ)
