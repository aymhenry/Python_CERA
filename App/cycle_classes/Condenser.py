...
class CondCool_Abstract(exf4Cond_Evap):
    ...
    def __init__(self, ICOND, objCP, objTrace):
        # set basic vars
        self.ATOTC = self.UA_FF_CND = self.UA_FZ_CND = self.UA_FF_HXS = None
        self.UA_FZ_HXS = self.CFMC = self.DTSUBC = None
        self.N_COND = self.TS1 = self.TS3 = self.TS5 = self.UDSC = None
        self.USCC = self.UTPC = self.Q_CND_FF = self.Q_CND_FZ = None
        self.Q_HXS_FF = self.Q_HXS_FZ = self.UACOND = self.Q_CND_FF = None
        self.Q_HXS_FF = self.Q_CND_FZ = self.Q_HXS_FZ = None
        self.UDSC = self.UTPC = self.USCC = self.UACOND = self.UACOND = self.UACOND = None
