...
class CycleSolver(CycleUtils):
    ...
    def __init__(self, objCP, dt, cab, lng_item, NCYC=1):
        # set basic vars
        self.FLOW = self.MREFSV = self.JC = self.LCCON = self.LQUIT = None
        self.TFF = self.TFZ = self.ICONC = self.IC = self.ITMAXC = None
        self.DUTYR = self.FSUPC = self.FSUPE = self.FSUBC = self.UACOND = None
        self.TS2 = self.JC = self.ICONC = self.ETAC = None
        self.IE = self.I_ERROR_INTER = self.HBUB15 = self.FF_FRACT = None
        self.QFRSH = self.UAFF = self.FSUPE = self.JE = None
        self.TS4 = self.LECON = self.TDEW = self.CPRVAP = self.QINT = None
        self.TS6 = self.QFREZ = self.W = self.QE = self.QC = self.QZ = None
        self.COPR = self.COPI = self.PR = self.TSUPC = self.TS = self.TENV = None
        self.TMID_COND = self.TMID_EVAP = self.CORR_COP = None
        # --------------------------------------------------
        # main init method
        # --------------------------------------------------
        self.objCP = objCP
    ...
    def condenser_calc(self):
        ...
        # Condenser Heat Fresh Food,                 Q_CND_FF watt
        # Condenser Heat Freezer,                    Q_CND_FZ watt
        # Heat Exchanger Fresh Food                  Q_HXS_FF watt
        # Heat Exchanger Freezer                     Q_HXS_FZ watt
        # Condenser                                  UACOND   watt/K
        # Desuperheating Heat Transfer Conductance,  UDSC     watt/m2 K
        # Subcooling Heat Transfer Conductance,      USCC     watt/m2 K
        # Two-Phase Heat Transfer Conductance,       UTPC     watt/m2 K
        ...
        self.ETAC = QTOTC / QMAXC   # watt/watt unitless
        # -----------------------------------------------------------
            ...
            if self.IC != 1:  # skip first trail to calc. some values later
                [QFF, QFZ, DUTYR] = \
                    self.dutfnd(self.dt, self.cab, self.dt.ICAB,
                                self.FANE,       # watt
                                self.cab.IRFTYP,
                                self.QFRSH,  # from Evaporator class watt
                                self.QFREZ,  # watt
                                self.cab.FROSTF,  # Fresh Food Door Frost Load  (watt)
                                self.cab.FROSTZ,  # Freezer Food Door Frost Load (watt)
                                self.TS3,     # K 
                                self.TS5,     # K
                                self.T,       # K
    ...
    def calc_lowevap(self):
        ...
        [self.H, self.P, self.T, self.TS6, self.QFREZ] = \
            self.lowevp(self.dt, self,
                        ...
                        self.TS3, self.dt.TS5, self.dt.DPF,  # pa
                        self.ETHX2
                        )
