...
class EvapCool_Abstract (exf4Cond_Evap):
    def __init__(self, IFRSH, objCP, objTrace):
        # Setup basic vars
        self.CFME = self.UTPE = self.USUPE = self.UA_FF = self.ATOTE = self.TS3 = None
        self.N_EVAP = self.TROOM = self.FZTEMP = self.Q_HXS_FF = self.IWALL_FF = self.NUM_ZONE = self.IRFTYP = None
