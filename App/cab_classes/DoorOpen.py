...
class DoorOpen(CabUtils):
    ...
    def __init__(self):
        self.par_HFFCS = self.par_HFFCL = None
    ...
    def main(self):
        ...
        # FF Condensation
        loc_QLFFC = self.com_FFCOPN * self.par_HFFCL * loc_VOLFFC * loc_RHOAIR \
            + self.com_WATERF * (1061.0 + 0.444 * loc_TFFCAB)
        loc_QSFFC = self.com_FFCOPN * self.par_HFFCS * loc_VOLFFC * loc_RHOAIR

        # FF Freezing
        loc_QLFFF = self.com_FFCOPN * loc_DHFFFL * loc_VOLFFC * loc_RHOAIR \
            + self.com_WATERF * ((1061.0 + 0.444 * loc_TFFCAB) + (158.9 - 0.488 * loc_TFFF))
        loc_QSFFF = self.com_FFCOPN * loc_DHFFFS * loc_VOLFFC * loc_RHOAIR

        # FZ Condensation
        loc_QLFZC = self.com_FRZOPN * loc_DHFZCL * loc_VOLFRZ * loc_RHOAIR \
            + self.com_WATERZ * (1061.0 + 0.444*loc_TFZCAB)
        loc_QSFZC = self.com_FRZOPN * loc_DHFZCS * loc_VOLFRZ * loc_RHOAIR

        # FZ Freezing
        loc_QLFZF = self.com_FRZOPN * loc_DHFZFL * loc_VOLFRZ * loc_RHOAIR \
            + self.com_WATERZ * ((1061.0 + 0.444*loc_TFZCAB) + (158.9 - 0.488*loc_TFRZE))
        ...
        if self.com_NMOD != 5.0:                                 # Upright door
            ...
            res8_QDFZFL = 0.25 * loc_QLFZF + 0.45 * loc_QNVFZF \
                + self.com_WATERZ * ((1061.0 + 0.444 * loc_TFZCAB) + (158.9 - 0.488*loc_TFRZE))
