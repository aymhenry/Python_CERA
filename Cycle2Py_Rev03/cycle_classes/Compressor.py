...
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
class Comp_Abstract (ABC, Block2):
	...
	def comp (self, H, P, X, T, MEFF, QHILO, QCAN, V, TAMB):
		...
		#
		# input	 H, P, X, T, CP, MEFF,  V, TAMB
		# output T, CV, CP, HOUT, QHILO, QCAN, VSUC, VV2, TSUC, TDISC, GAMA, RN, ETAS
		#		[P2, P4, P5, P6, P7 , P9 to P11,  P14, P15, P17, P18,P19 ] = self.comp (P1 to P4, P8, P9, P10, P12, P16 )
		#     *****************************************************************
		#     *    COMPRESSOR MODEL                                           *
		#     *****************************************************************
		#
		...
		#          SELECT MODEL
		#
		if(Data.IMAP == 1) :                                 ###Map model
			if(Data.ICOOL == 0) :
				TSUC = 389.59 - 64.815 * Data.EER                     ###Degrees F
			else:
				TSUC = 337.84 - 57.895 * Data.EER
			#End if
			...
			if(Data.ICOMP == 2): TSUC = TIN + 30.0                ###Rotary
			...
			if(Data.ICOMP == 2) :
				ETAS = Data.EFFC * (1.0 - 0.0010*(T_COND - 130.0)) * (1.0 + 0.0030*(T_EVAP + 10))
			else:
				ETAS = Data.EFFC * (1.0 + 0.0010*(T_COND - 130.0)) * (1.0 + 0.0020*(T_EVAP + 10))
			#End if


			W = (H2S - H_SUC)/Data.EFFC
			if(Data.ICOOL == 1): W = (H2S - H_SUC)/ETAS
			...
			if(Data.ICOOL == 0) :
				RATIO = 0.68 - 0.05 * Data.EER
			else:
				RATIO = 0.90 - 0.07 * Data.EER
			#End if
		...
		#
		if(Data.ICOMP == 1) :
			ETAV = 0.92*(1.0 - Data.CE*(PR**RINV - 1.0))
		else:
			ETAV = 1.00*(1.0 - Data.CE*(PR**RINV - 1.0))
		#End if
		Data.DISP = Data.MREF * VSUC/(60.0 * Data.SPEED * ETAV)
...
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
class Comp_Map (Comp_Abstract): #Data.IMAP== 0
	def comp_balance (self):
		OLDMAS = Data.MREF
	...
	def compcall(self, H,P,X,T,V,   TAMB):
		...
		#
		WDOTS = Data.MREF*(H2S-H[1])
		#
		#          DETERMINE ACTUAL COMPRESSOR PERFORMANCE
		#
		#[ P8, P9, P10, P11 , P12] = self.compmap (P1..P7 )
		[ TSUC, WDOT, Data.MREF, QSHELL, Data.SPEED] = self.compmap (P[1], P[2], T[1], V[1], TAMB, X, GAMA )
		#CALL COMPMAP(P[1], P[2], T[1], V[1], TAMB, X, GAMA, TSUC, WDOT, Data.MREF, QSHELL, Data.SPEED)
		#
		#          CALCULATE REFRIGERANT EXIT ENTHALPY AND TEMPERATURE
		#
		fact = QSHELL / WDOT  # useless not used anywhere
		
		H[2] = H[1] + (WDOT-QSHELL)/Data.MREF
	...
	def compmap (self, PSUCT, PDISC, TSUCT, VSUCT, TAMB, X, GAMA):
		...
		if (Data.IREAD == 0) :
			...
			# Data.ICOMP : I1 compressor type (1 - reciprocating; 2 - rotary) 
			...
			Data.ICOMP = inastk ( objCompMap, "int" ) 
		...
		#
		if  (Data.ICOMP  ==  1) :                    # Reciprocating compressor
			DELTIN = 67.0
...
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
class Comp_ERR (Comp_Abstract): #Data.IMAP== 1
	def comp_balance (self):
		...
		Data.MREF = (Data.MREF * (self.objData.DISPLC/DISPI) + 2.0*OLDMAS)/3.0
	...
	def map (self, ICOMP, ICOOL, EER, SIZE, DISPL , SPEEDN):
		...
		for I in range (1, 3+1): #DO I = 1, 3
			for J in range (1, 3+1): #DO J = 1, 3
				Data.ETAC = Data.ETAC + ETAS[I][J]
				Data.CE   = Data.CE   + CEIJ[I][J]
			#END DO
		#END DO

		Data.ETAC = Data.ETAC/9.0
		Data.CE   = Data.CE/9.0
...
# Editor		: aymhenry@gmail.com
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
class Comp_EMOD (Comp_Abstract): #Data.IMAP== 2
	def comp_balance (self):
		# CALL THEORETICALLY BASED MODEL
		lstRes = self.comp (self.objData.H, self.objData.P, 	 self.objData.X, \
						  self.objData.T, self.objData.MEFF, self.objData.QHILO, \
