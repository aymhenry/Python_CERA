...
class Comp_Abstract (ABC, Block2, Data):
    ...
    def comp(self, H, P, X, T, MEFF, QHILO, QCAN, V, TAMB):
        ...
        # XV= [[0.0] * (16+1) for i in range(5+1)]        # array(Rows, Cols) =
        ...
        if(XQ[2] < 1.0):
            ...
            [HL2, CV, CP, VS] = self.hcvcps(1, T[2], VL2, XL)
            # CALL HCVCPS (3,T[2],VV2, XV[1][2],HV2, CV,CP,VS)
            [HV2, CV, CP, VS] = self.hcvcps(3, T[2], VV2, XV)

            H[2] = XQ[2] * HV2 + (1.0 - XQ[2]) * HL2
        else:
            # CALL HCVCPS (3,T[2],VV2,X,H[2],CV,CP,VS)
            [H[2], CV, CP, VS] = self.hcvcps(3, T[2], VV2, X)
        # End if
        HISEN = H[2]
        #
        #          SELECT MODEL
        ...
        if(Data.obj_cdata.IMAP == 1):  # Map model
            ...
            # [P4 .. P11] = self.spin (P1, P2, P3 )
            [T2S, XQ[2], XL, XV, VL2S, VV2S, SL2S,
                SV2S] = self.spin(SSUC, P[2], X)
            # CALL SPIN(SSUC, P[2], X, T2S, XQ[2], XL[1][2], XV[1][2], VL2S,
            # VV2S, SL2S, SV2S)
            ...
            if(Data.obj_cdata.ICOMP == 2):
                ETAS = Data.obj_cdata.EFFC * \
                    (1.0 - 0.0010 * (T_COND - 130.0)) * (1.0 + 0.0030 * (T_EVAP + 10))
            else:
                ETAS = Data.obj_cdata.EFFC * \
                    (1.0 + 0.0010 * (T_COND - 130.0)) * (1.0 + 0.0020 * (T_EVAP + 10))
            # End if

            W = (H2S - H_SUC) / Data.obj_cdata.EFFC
            if(Data.obj_cdata.ICOOL == 1):
                W = (H2S - H_SUC) / ETAS
            ...
            [H[2], CV, CP, DUM1] = self.hcvcps(1, T[2], VV2, X)
            ...
            while (ERROR > TOLS and ITER < 10):
                ...
                SSUC = self.entrop(TSUC, VSUC, X)
                # [P4 .. P11] = self.spin (P1, P2, P3 )
                [T[2], XQ[2], XL, XV, VL2, VV2, SL,
                    SV] = self.spin(SSUC, P[2], X)
                ...
                COEF1 = (1.0 - MEFF - (MEFF * QCAN - QHILO) /
                         (1.0 - QCAN)) / (1.0 + QHILO / (1.0 - QCAN))
                H1P = H[1] + COEF1 * (H[2] - H[1])

                [T1P, XQ1, XL, XV, VL2, VV2, HL2,
                    HV2] = self.hpin(H1P, P[1], X)
            ...
            [TDISC, XQ[2], XL, XV, VL2, VV2, HL2,
                HV2] = self.hpin(HDISC, P[2], X)

            # CALL HPIN(HDISC, P[2], X, TDISC, XQ[2], XL[1][2], XV[1][2], VL2,
            # VV2, HL2, HV2)
            H[2] = H[1] + ((H[2] - H[1])) / (1.0 + QHILO / (1.0 - QCAN))

            [T[2], XQ[2], XL, XV, VL2, VV2, HL2,
                HV2] = self.hpin(H[2], P[2], X)
            # CALL HPIN(H[2], P[2], X,   T[2], XQ[2], XL[1][2], XV[1][2], VL2,
            # VV2, HL2, HV2)

        #          CALCULATE MASS FLOW RATE
        if(Data.obj_cdata.ICOMP == 1):
            Data.obj_cdata.ETAV = 0.92 * \
                (1.0 - Data.obj_cdata.CE * (PR**RINV - 1.0))
        else:
            Data.obj_cdata.ETAV = 1.00 * \
                (1.0 - Data.obj_cdata.CE * (PR**RINV - 1.0))

        Data.obj_cdata.DISP = Data.obj_cdata.MREF * VSUC / \
            (60.0 * Data.obj_cdata.SPEED * Data.obj_cdata.ETAV)
...
class Comp_Map (Comp_Abstract):  # Data.obj_cdata.IMAP== 0
    ...
    def comp_balance(self):
        ...
        if (Data.obj_cdata.MREF > 1.05 * OLDMAS):
            Data.obj_cdata.MREF = 1.05 * OLDMAS
        if (Data.obj_cdata.MREF < 0.95 * OLDMAS):
            Data.obj_cdata.MREF = 0.95 * OLDMAS
    ...
    def compmap(self, PSUCT, PDISC, TSUCT, VSUCT, TAMB, X, GAMA, SPEED):
        ...
        if (Comp_Map.IREAD is None):
            ...
            # TCDATA[I] = inastk ( objCompMap, "float" ) # Python commnet:
            # first number is COND TEMP (y axis)

            #	for J in range (1, NEVAP+1 ):
            #		CAPAC[I][J] = inastk ( objCompMap, "float" )

            # for I in range (1, NCOND +1) : #DO I = 1,NCOND
            # DUMMY = inastk ( objCompMap, "float" ) # Python commnet: first
            # number is COND TEMP (y axis)
...
class Comp_ERR (Comp_Abstract):  # Data.obj_cdata.IMAP== 1
    def comp_balance(self):
        ...
        Data.obj_cdata.MREF = (
            Data.obj_cdata.MREF * (self.objData.DISPLC / DISPI) + 2.0 * OLDMAS) / 3.0
        if (Data.obj_cdata.MREF > 1.05 * OLDMAS):
            Data.obj_cdata.MREF = 1.04 * OLDMAS
        if (Data.obj_cdata.MREF < 0.95 * OLDMAS):
            Data.obj_cdata.MREF = 0.96 * OLDMAS
        return lstRes
    ...
    def map(self, ICOMP, ICOOL, EER, SIZE, DISPL, SPEEDN):
        ...
        for I in range(1, 3 + 1):  # DO I = 1, 3
            ...
            for J in range(1, 3 + 1):  # DO J = 1, 3
                ...
                # CALL SPIN(SSUC, P_COND, X, T2S, XQ, XL, XV, VL2S, VV2S, SL2S,
                # SV2S)
