...
class CycleType_Abstract (ABC, CycleUtil):
    ...
    def adjust_input_for_all_types(self):
        ...
        self.obj_data.FILMAP1 = self.getCompressorFileName(
            self.obj_data.FILMAP1_CODE) + ".cmp"
        ...
        # Automatic Defrost (W)
        if(self.obj_data.IDFRST == 1):
            self.obj_data.DFSTCYC = 0.0
    ...
    def adjust_units_for_all_types(self):
        ...
        # fixed not the same FORTAN
        self.obj_data.CONDHT[1] = 3.6 * self.obj_data.CONDHT[1]
        # fixed not the same FORTAN
        self.obj_data.CONDHT[2] = 3.6 * self.obj_data.CONDHT[2]

        # fixed not the same FORTAN
        self.obj_data.CONDVP[1] = 3.6 * self.obj_data.CONDVP[1]
        # fixed not the same FORTAN
        self.obj_data.CONDVP[2] = 3.6 * self.obj_data.CONDVP[2]
    ...
    def prepare_Data4Cycle(self, lng_item):
        ...
        if(self.obj_data.TSPECI[lng_item] > 0.0):
            self.obj_data.TSPEC = self.obj_data.TSPECI[lng_item] + 273.11
        ...
        print("aym --- self.obj_data.SPEEDN", self.obj_data.SPEEDN)
        ...
        self.obj_data.CFMC = 1.8961 * \
            (RHOCPC * self.obj_data.CFMCI[lng_item]) / 0.4720
        self.obj_data.CFME = 1.8961 * \
            (RHOCPE * self.obj_data.CFMEI[lng_item]) / 0.4720
    ...
    def call_cycle(self, lng_item):
        ...
        # both the same value
        obj_parameter_data.ETHX1 = self.obj_data.ETHX[lng_item]
        ...
        # number of call to cycle (1=Single or 2= Dual cycle)
        obj_parameter_data.NCYC = 1
...
class Type_3DualLoop (CycleType_Abstract):
    ...
    def adjust_input(self):
        ...
        # get compressor file name
        self.obj_data.FILMAP2 = self.getCompressorFileName(
            self.obj_data.FILMAP2_CODE)
