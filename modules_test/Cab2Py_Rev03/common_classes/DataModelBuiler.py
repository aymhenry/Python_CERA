...
class DataModelBuiler (FileAccess):
	...
	def assign_vars (self):
		lst_config1 = ['IRFTYP','HEIGHT','WIDTH','DEPTH','WEDGE','FLANGE','WEDGER','FLANGER','DOOREDG','DGSKT','CDUP','CDDN','CCHGT','DOL','COL','DIL','CIL','TOPMUL','THMUL','TIFT','TIFLS','TIFRS','TIFF','TIFB','TIRLS','TIRRS','TIRF','TIRB','BINSUL','CINSUL','HXVUZ','VOLAZ','HXVUR','VOLAR','TROOM','TFRZ','TFF','TBTM','RFF','RFRZ','RWEDGER','RWEDGE','RDRFF','RDRFZ','RMUL','HLFZG','HLRG','TDRAIR','RELHUM','FFCOPN','SECFFC','FRZOPN','SECFRZ','FFASHW','FZASHW','FFAUXW','FZAUXW','OTHERW','FFPENA','FZPENA','FFASHQ','FZASHQ','FFREFQ','FZREFQ','FFHEAT','FZHEAT']
		lst_config2 = ['IRFTYP','HEIGHT','WIDTH','DEPTH','WEDGE','FLANGE','WEDGER','FLANGER','DOOREDG','DGSKT','CDUP','CDDN','CCHGT','DOL','COL','DIL','CIL','TOPMUL','THMUL','TIFRS','TIFF','TIFB','TIRT','TIRLS','TIRF','TIRB','BINSUL','HXVUZ','VOLAZ','HXVUR','VOLAR','TROOM','TFRZ','TFF','TBTM','RFF','RFRZ','RWEDGER','RWEDGE','RDRFF','RDRFZ','RMUL','HLGZF','HLRG','TDRAIR','RELHUM','FFCOPN','SECFFC','FRZOPN','SECFRZ','FFASHW','FZASHW','FFAUXW','FZAUXW','OTHERW','FFPENA','FZPENA','FFASHQ','FZASHQ','FFREFQ','FZREFQ','FFHEAT','FZHEAT']
		lst_config3 = ['IRFTYP','HEIGHT','WIDTH','DEPTH','WEDGE','FLANGE','WEDGER','FLANGER','DOOREDG','DGSKT','CDUP','CDDN','CCHGT','DOL','COL','DIL','CIL','WALL','THMUL','TIFT','TIFRS','TIFF','TIFB','TIRT','TIRLS','TIRF','TIRB','BINSUL','CINSUL','BINFRZ','HXVUZ','VOLAZ','HXVUR','VOLAR','TROOM','TFRZ','TFF','TBTM','RFF','RFRZ','RWEDGER','RWEDGE','RDRFF','RDRFZ','RMUL','HLGZF','HLRG','TDRAIR','RELHUM','FFCOPN','SECFFC','FRZOPN','SECFRZ','FFASHW','FZASHW','FFAUXW','FZAUXW','OTHERW','FFPENA','FZPENA','FFASHQ','FZASHQ','FFREFQ','FZREFQ','FFHEAT','FZHEAT']
		lst_config4 = ['IRFTYP','HEIGHT','WIDTH','DEPTH','DOOREDG','DGSKT','CWIDE','CHGT','DOL','COL','DIL','CIL','TIFT','TIFRS','TIFF','TIFB','BINSUL','CINSUL','SCIN','STIN','HXVUZ','VOLAZ','TROOM','TFRZ','TBTM','RCAB','RTOP','HLFZG','TDRAIR','RELHUM','FRZOPN','SECFRZ','FZASHW','FZAUXW','OTHERW','FZPENA','FZASHQ','FZREFQ','FZHEAT']
		lst_config5 = ['IRFTYP','HEIGHT','WIDTH','DEPTH','WEDGE','FLANGE','DOOREDG','DGSKT','CDUP','CDDN','CCHGT','DOL','COL','DIL','CIL','TIFT','TIFLS','TIFRS','TIFF','TIFB','BINSUL','CINSUL','HXVUZ','VOLAZ','TROOM','TFRZ','TBTM','RCAB','RWEDGE','RDOOR','HLFZG','TDRAIR','RELHUM','FRZOPN','SECFRZ','FZASHW','FZAUXW','OTHERW','FZPENA','FZASHQ','FZREFQ','FZHEAT']
		lst_config6 = ['IRFTYP','HEIGHT','WIDTH','DEPTH','WEDGE','FLANGE','DOOREDG','DGSKT','CDUP','CDDN','CCHGT','DOL','COL','DIL','CIL','TIFT','TIFLS','TIFRS','TIFF','TIFB','BINSUL','CINSUL','HXVUZ','VOLAZ','HXVUR','VOLAR','FH','FW','FD','TROOM','TFRZ','TFF','TBTM','RCAB','RWEDGE','RDOOR','HLFZG','TDRAIR','RELHUM','FRZOPN','SECFRZ','FZASHW','FZAUXW','OTHERW','FZPENA','FZASHQ','FZREFQ','FZHEAT']
	...
	def err_description (self):
		...
		if self.m_error == DataModelBuiler.ERR_FOUND:
			...
			return "Err " + str(DataModelBuiler.ERR_FOUND_READLINE) + self.m_error_desc
			...
			return "No error descriptio, info. number DataModelBuiler:" + str(self.m_error)
	...
	def chk_data_model (self):
		...
		# if error is returned from the above method, return with Flase, with error code set by getData_from_list
		if self.isError(): 
		...
		# check configration is in the accaptable range as setting in the current class, if error return False
	...
	def readlines (self):		
		...
		try:
			...
			while True :
				...
				# check that lines is not more this max. value, this for safety only
				...
				self.lst_data.append (num_list[0]) # append first col,to the main list

		except OSError as err_desc:
			self.m_error = DataModelBuiler.ERR_FOUND_READLINE
			self.m_error_desc = str(err_desc) + " line" + str(self.m_lineNo) # this var in super class FileAccess
	...
	def getData_from_list (self, int_row_number, strType = ""  ):
		...
		if int_row_number > len(self.lst_data) or int_row_number < 0: # exit if error entry
			#set Error Flag
		...
		# count then number of (.) and (-) in text, check that no more than one(-) and one  "." only.
		n_count_muns = str_text.count("-")
		n_count_dec = str_text.count(".")

		# check that the remaning text is number, after removeing all (.)s and (-)s
